...
function sourceLabel(value) {
  return ({
    official: "公式のみ",
    pc_kyotei: "PC-KYOTEIのみ",
    mixed: "公式＋PC補完",
    normal_only: "通常展示のみ",
    kiryu_official_rank: "桐生公式順位",
    unknown: "収集元未判定",
  })[value] || "収集元未記録";
}
  ...
  try {
    const [todayCount, notifiedCount, last30Count, recentResult] = await Promise.all([
      ...
      client.from(theory.table)
        .select("race_date,course_code,course_name,race_no,detected_at,notified,notified_at,exhibition_source_kind,exhibition_synced_at")
...
export { loadStatus, buildAlerts, statusTone, statusText, formatJst, countText, sourceLabel, durationText };
