...
function MetricCard({ label, value, suffix = "" }) {
  return <div className={styles.metricCard}><span>{label}</span><strong>{value}{value !== "—" && suffix ? <small>{suffix}</small> : null}</strong></div>;
}
...
function BetCard({ item }) {
  ...
  return (
    <article className={styles.betCard}>
      <div className={styles.betTop}><strong>{item.courseName}{item.raceNo}R</strong>{item.isHit == null ? null : <span className={item.isHit ? styles.hitBadge : styles.missBadge}>{item.isHit ? "的中" : "不的中"}</span>}</div>
      <div className={styles.betSectionHead}><strong>買い目</strong><span>（{tickets.length}点）</span></div>
      <div className={styles.ticketGrid}>{tickets.length ? tickets.map((ticket) => <TicketChip key={ticket} ticket={ticket} isHit={ticket === hitTicket} stake={item.stakes?.[ticket]} />) : <span className={styles.emptyTickets}>—</span>}</div>
      <div className={styles.betMeta}><span>{item.characterLabel}</span>{item.investment ? <span>投資 {formatYen(item.investment)}</span> : null}{item.isHit ? <span className={styles.payoutMeta}>払戻 {formatYen(item.payout)}</span> : null}</div>
    </article>
  );
}
    ...
    <div className={styles.wrapper}>
      ...
      <div ref={sliderRef} className={styles.slider} onScroll={handleScroll}>
        {MODES.map((mode) => {
          ...
          const visibleBets = !isExpanded ? bets.slice(0, 4) : bets.slice(0, 8);
