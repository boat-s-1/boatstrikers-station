create table if not exists public.bs_ichika_hidden_escape_alerts (
  ...
  exhibition_best_time numeric,
  exhibition_gap numeric,
  lap_time numeric,
  lap_rank smallint,
  ...
  constraint bs_ichika_hidden_escape_alerts_race_unique unique (race_date, course_code, race_no)
);

create index if not exists idx_bs_ichika_hidden_escape_alerts_pending
  on public.bs_ichika_hidden_escape_alerts (race_date, notified, detected_at);
...
begin
  with ranked as (
    select
      ...
      e.official_lap,
      rank() over (
        partition by e.race_date, e.course_code, e.race_no
        order by e.official_exhibition_time asc nulls last
      )::smallint as exhibition_rank,
      rank() over (
        partition by e.race_date, e.course_code, e.race_no
        order by e.official_lap asc nulls last
      )::smallint as lap_rank,
      min(e.official_exhibition_time) over (
        partition by e.race_date, e.course_code, e.race_no
      ) as exhibition_best_time
...
grant execute on function public.evaluate_ichika_hidden_escape_alerts() to service_role;
