...
export default function BetaMemberPlacementBanner() {
  ...
  return createPortal(
    <div style={{maxWidth:isRaces?1180:1120,margin:isRaces?"18px auto":"10px auto 18px",padding:isRaces?"0 14px":"0 8px",boxSizing:"border-box"}}>
      <div style={{position:"relative",display:"block",minHeight:isRaces?250:300,overflow:"hidden",borderRadius:isRaces?24:28,color:"#fff",background:"radial-gradient(circle at 82% 16%, rgba(244,127,255,.52), transparent 28%), radial-gradient(circle at 12% 92%, rgba(0,235,255,.38), transparent 31%), linear-gradient(118deg,#05267e 0%,#0754d6 45%,#7144e6 100%)",boxShadow:"0 14px 36px rgba(15,63,145,.28)",border:"1px solid rgba(255,255,255,.35)",isolation:"isolate"}}>
        <div aria-hidden="true" style={{position:"absolute",inset:0,opacity:.28,backgroundImage:"radial-gradient(circle, rgba(255,255,255,.75) 1px, transparent 1.5px)",backgroundSize:"18px 18px",maskImage:"linear-gradient(90deg,#000 0%,transparent 65%)",zIndex:-1}} />

        <div style={{position:"absolute",right:14,top:14,zIndex:3,padding:"8px 13px",borderRadius:999,color:"#073b9e",background:"linear-gradient(180deg,#fff8a8,#ffd64e)",border:"2px solid rgba(255,255,255,.9)",boxShadow:"0 5px 16px rgba(28,34,116,.24)",fontSize:"clamp(13px,2.8vw,18px)",fontWeight:1000,whiteSpace:"nowrap"}}>12/31まで無料</div>

        <div className="betaMembershipGrid" style={{position:"relative",zIndex:2,display:"grid",gridTemplateColumns:isRaces?"minmax(0,1.15fr) minmax(220px,.85fr)":"minmax(0,1.1fr) minmax(240px,.9fr)",gap:12,alignItems:"end",minHeight:isRaces?250:300,padding:isRaces?"24px 24px 22px":"28px 28px 26px",boxSizing:"border-box"}}>
          <div style={{minWidth:0,alignSelf:"center"}}>
            ...
            <p style={{margin:0,maxWidth:620,fontSize:"clamp(12px,2.5vw,17px)",lineHeight:1.65,fontWeight:800,color:"rgba(255,255,255,.96)"}}>BS展示・直前版AI・直前買い目など、会員限定機能をβメンバー向けに順次開放します。</p>

            <div className="betaMembershipActions" style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginTop:16}}>
          ...
          <div aria-hidden="true" className="betaMembershipCharacters" style={{display:"flex",alignItems:"flex-end",justifyContent:"center",gap:0,minHeight:170,paddingTop:54}}>
            {CHARACTERS.map((character,index)=>(
              <div key={character.alt} style={{width:isRaces?118:138,aspectRatio:"1 / 1.18",marginLeft:index?-26:0,borderRadius:"44% 44% 18% 18%",overflow:"hidden",border:"3px solid rgba(255,255,255,.72)",boxShadow:"0 10px 24px rgba(13,25,96,.28)",transform:index===1?"translateY(-13px) scale(1.04)":"translateY(0)",background:"rgba(255,255,255,.18)"}}><img src={character.src} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} /></div>
            ))}
          </div>
        </div>
        ...
        <style jsx>{`
          @media (max-width:720px){
            .betaMembershipGrid{grid-template-columns:1fr !important;min-height:0 !important;padding:22px 18px 18px !important}
            .betaMembershipCharacters{position:absolute !important;right:8px;bottom:-8px;width:47%;min-height:0 !important;padding-top:0 !important;opacity:.42;pointer-events:none}
            .betaMembershipCharacters>div{width:78px !important;border-width:2px !important}
            .betaMembershipGrid>div:first-child{position:relative;z-index:2;padding-right:3%}
            .betaMembershipActions{gap:8px !important}
