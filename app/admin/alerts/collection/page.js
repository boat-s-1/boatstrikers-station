...
export default async function CollectionPage({ searchParams }) {
  ...
  return <main className={styles.page}><div className={styles.shell}>
    ...
    {!rows ? <p role="alert" className={styles.notice}>保存データを読み込めませんでした。「未取得」とは判定していません。再読込してください。</p> : <>
      ...
      <p className={styles.notice}>取得記録は今回の追加以降、自動収集で確認したレースだけに残ります。締切前の収集対象時間外・記録開始前の未取得理由は分かりません。「取得・6艇保存成功」は保存まで完了した状態です。保存値は「更新済R」でも確認できます。個別の読み取り専用診断は記録対象外です。</p>
    </>}
  </div></main>;
}
