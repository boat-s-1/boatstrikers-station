...
test('diagnostics only enabled for connected verified or staging adapters',()=>assert.deepEqual(VENUES.filter(v=>v.endpoint).map(v=>v.code),[5,12,14,15,18,19,20,23,24]));
test('staging never labelled connected',()=>assert.ok(VENUES.filter(v=>[12,24].includes(v.code)).every(v=>v.stage==='検証用実装')));
