...
const racerName = (value) => clean(value).replace(/[\s\u3000]+/g, '');
...
export function verifyNationalBeforeInfo(html, race) {
  ...
  for (const match of table[0].matchAll(/<tbody\b[^>]*>([\s\S]*?)<\/tbody>/gi)) {
    ...
    const name = racerName(rowCells[2][2]);
    if (!name) return { ok: false, error: "reference_boat_identity_invalid", rows: [] };
    rows.push({ boatNo: Number(boat), racerNo: ids[0], racerName: name, exhibitionTime });
  }
