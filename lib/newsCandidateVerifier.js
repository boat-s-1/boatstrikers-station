...
export async function verifyNewsCandidate(item, { client = getClient(), now = new Date() } = {}) {
  ...
  let resultStatus = cancelled ? "cancelled" : resultAvailable ? "confirmed" : deadline && !deadlinePassed ? "scheduled" : "unknown";
  ...
  if (resultClaim) {
    ...
    verificationStatus = "verified";
    note = resultAvailable ? "race_fact_confirmed_by_bs_race_events" : "scheduled_race_confirmed_by_bs_race_events";
    xCandidate = true;
