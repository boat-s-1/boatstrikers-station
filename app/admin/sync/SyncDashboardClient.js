...
export default function SyncDashboardClient() {
  ...
  async function syncRacers(){ setBusy(true); setMessage(""); try{ const r=await fetch("/api/admin/sync/racers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({limit:4})}); const j=await r.json(); const failedNames=(j.failed||[]).map(x=>x.name||x.registration_no).filter(Boolean); setMessage(r.ok||r.status===207?`選手マスタ同期: ${j.synced||0}人追加 / 合計${j.total_profiles||0}人${failedNames.length?` / 失敗: ${failedNames.join("、")}`:""}`:j.error||"選手マスタ同期に失敗しました"); }catch(error){ setMessage("選手マスタ同期に失敗しました。通信状態を確認して再度お試しください。"); }finally{ setBusy(false); } }
  async function syncRacersBulk(){ setBusy(true); setMessage("選手マスタ連続同期を開始します…"); const failedMap=new Map(); let added=0; let totalProfiles=0; try{ for(let batch=1;batch<=13;batch++){ setMessage(`選手マスタ連続同期: ${added}/50人追加済み（${batch}/13回目）`); let response=null; let payload=null; for(let attempt=1;attempt<=2;attempt++){ response=await fetch("/api/admin/sync/racers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({limit:4,exclude:[...failedMap.keys()]})}); try{ payload=await response.json(); }catch{ payload={error:"invalid_response"}; } if(response.ok||response.status===207) break; if(attempt===1){ setMessage(`選手マスタ連続同期: ${added}/50人追加済み（${batch}/13回目を再試行中…）`); await new Promise(resolve=>setTimeout(resolve,1200)); } } if(!(response?.ok||response?.status===207)) throw new Error(payload?.error||"sync_failed"); added+=Number(payload.synced||0); totalProfiles=Number(payload.total_profiles||totalProfiles); for(const item of payload.failed||[]){ if(item?.registration_no) failedMap.set(item.registration_no,item.name||item.registration_no); } if(!payload.candidates||added>=50) break; } const failedNames=[...failedMap.values()]; setMessage(`選手マスタ連続同期完了: ${added}人追加 / 合計${totalProfiles}人${failedNames.length?` / 今回スキップ: ${failedNames.join("、")}`:""}`); }catch(error){ const failedNames=[...failedMap.values()]; setMessage(`選手マスタ連続同期を途中終了: ${added}人追加${failedNames.length?` / スキップ: ${failedNames.join("、")}`:""}`); }finally{ setBusy(false); } }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.panel}><div className={styles.panelTitle}><div><p>MANUAL CONTROL</p><h2>手動同期</h2></div><span>指示はSupabaseに登録され、Windowsエンジンが処理します。</span></div><div className={styles.commandGrid}>{COMMANDS.map(([key,label])=><button disabled={busy||r.state==="running"} key={key} onClick={()=>command(key)}>{label}</button>)}<button disabled={busy} onClick={syncRacers}>{busy?"同期中…":"選手マスタ4人同期"}</button><button disabled={busy} onClick={syncRacersBulk}>{busy?"同期中…":"選手マスタ50人連続同期"}</button></div></section>
