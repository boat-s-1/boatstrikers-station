...
export async function fetchMikuniVerifiedOriginalTenji(race, options = {}) {
  ...
  try {
    const response = await fetch(url, { cache: "no-store", redirect: "manual", signal: controller.signal, headers: { "user-agent": "BoatStrikers-ExhibitionDiagnostic/1.0", "x-requested-with": "XMLHttpRequest", referer: "https://www.mikuniks-web.jp/" } });
    if (!response.ok) { await response.body?.cancel(); return { ...fail("upstream_http_error"), status: response.status, url }; }
    const html = await response.text();
    if (!/<title>\s*BOAT RACE 三国 #10 /i.test(html)) return { ...fail("source_venue_mismatch"), url };
    const month = Number(raceDate.slice(5, 7)), day = Number(raceDate.slice(8));
    const dateOk = new RegExp(`${month}\\/${day}\\([^)]+\\)`).test(clean(html).slice(0, 1000));
    const raceOk = new RegExp(`<option\\b(?=[^>]*\\bselected\\b)(?=[^>]*value=['"]${raceNo}['"])(?=[^>]*name=['"]\\/races\\/${raceNo}['"])[^>]*>`, "i").test(html);
    if (!dateOk || !raceOk) return { ...fail("source_identity_mismatch"), url };
    const rows = parseMikuniOfficialOriginalTenji(html);
    if (rows.length !== 6) return { ...fail("official_original_tenji_not_available"), url };
    return {
      ok: true, supported: true, published: true, source: "mikuni_official_verified", sourceLabel: "BOAT RACE三国公式",
      rows, url,
      identity: { verified: true, courseCode: 10, raceDate, raceNo, evidence: "official_date_selected_race_plus_agreeing_desktop_mobile_six_boat_racer_tables" },
