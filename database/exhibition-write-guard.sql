...
declare
  ...
  accepted jsonb := '{}'::jsonb; source_name text; item jsonb;
begin
  ...
  foreach field in array fields loop
    ...
    if new_valid and not stale then
      if not old_valid or new_value is distinct from old_value then
        source_name := coalesce(p_new->>source_field,p_new->>'data_source','unknown');
    ...
    if field in ('official_exhibition_st','exhibition_st') and (not new_valid or stale) then
