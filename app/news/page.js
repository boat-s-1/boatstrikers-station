...
function textOf(item){return `${item?.list_headline||""} ${item?.title||""} ${item?.summary||""} ${item?.category||""} ${item?.source_name||""}`}
function headlineOf(item){return String(item?.list_headline||item?.title||"ニュース").trim()}
function bucket(item){const t=textOf(item),c=String(item?.category||""),source=String(item?.source_name||"");if(/BoatStrikers|AI編集部|BSオリジナル/i.test(source)||/BoatStrikers独自|独自分析|独自記事/.test(t))return"bs";if(["result","win"].includes(c)||/結果|優勝|優出|V達成|初優勝|レース後|払戻|着順/.test(t))return"result";if(["motor","tomorrow"].includes(c)||/明日|翌日|あす|注目レース|高モーター|モーター|機力|展示|2連対率|優勝戦|初日/.test(t))return"focus";if(c==="grade"||/SG|G1|GⅠ|G2|GⅡ|グランプリ|周年|グレードレース/.test(t))return"grade";if(c==="women"||/女子|ヴィーナス|オールレディース|レディース|クイーンズ/.test(t))return"women";if(c==="suijinsai"||/選手|レーサー|水神祭|昇級|A1|A2|インタビュー|トークショー|結婚|引退|復帰|記録達成/.test(t))return"racer";return"all"}
...
export default async function NewsTopPage({searchParams}){
 const params=await searchParams;const category=TABS.some(x=>!x.href&&x.key===params?.category)?params.category:"all";const q=String(params?.q||"");const allNews=await getHatsuneNews({limit:100,category:"all"});const news=allNews.filter(i=>matches(i,category,q));const featured=news.find(i=>i.is_featured)||news[0]||null;const latest=news.filter(i=>i.id!==featured?.id).slice(0,14);const women=allNews.filter(i=>bucket(i)==="women").slice(0,6);const focus=allNews.filter(i=>bucket(i)==="focus").slice(0,6);const results=allNews.filter(i=>bucket(i)==="result").slice(0,6);
