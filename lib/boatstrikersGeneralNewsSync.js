...
const MAX_REPAIR_FETCHES_PER_RUN = 8;
...
function isUsableImageUrl(value) {
  ...
  if (/(?:logo|icon|sprite|avatar|favicon|blank|noimage|no-image|placeholder)/i.test(url)) return false;
  return true;
}
...
function extractCandidates(html, source) {
  ...
  while ((match = anchorRegex.exec(html))) {
    ...
    if (!category || WOMEN_RE.test(label)) continue;
    rows.push({
      title: label,
      summary: "",
      category,
...
async function enrichCandidate(item) {
  try {
    ...
    const imageUrl = extractArticleImage(html, item.source_url);
    return { ...item, ...(summary ? { summary } : {}), ...(imageUrl ? { image_url: imageUrl } : {}) };
  } catch {
    return item;
  }
}
...
async function repairExistingGeneralRows(supabase) {
  const { data, error } = await supabase
    .from("hatsune_news")
    .select("id,title,category,source_key,source_url,is_featured,priority,summary,article_body_source,image_url")
  ...
  const rows = data || [];
  const fetchTargets = rows
    .filter((row) => row.source_url && (!row.image_url || !normalizeSpace(row.summary) || /公開されたボートレース関連ニュースです|元記事を転載せず/.test(String(row.summary || ""))))
    .slice(0, MAX_REPAIR_FETCHES_PER_RUN);

  const enrichedMap = new Map();
  const enriched = await Promise.all(fetchTargets.map(async (row) => {
    try {
      const html = await fetchHtml(row.source_url);
      return [String(row.id), { summary: extractArticleSummary(html), image_url: extractArticleImage(html, row.source_url) }];
    } catch {
      return [String(row.id), null];
    }
  }));
  for (const [id, value] of enriched) if (value) enrichedMap.set(id, value);
  ...
  for (const row of rows) {
    ...
    const fetched = enrichedMap.get(String(row.id));
    if (fetched?.summary && (!normalizeSpace(row.summary) || genericSummary)) {
      patch.summary = fetched.summary;
      summaryRepairs += 1;
    } else if (genericSummary && !fetched?.summary) {
      patch.summary = "";
    }
    if (fetched?.image_url && !row.image_url) {
      patch.image_url = fetched.image_url;
      imageRepairs += 1;
    }
  ...
  return { repaired, imageRepairs, summaryRepairs };
}

export async function syncBoatstrikersGeneralNews() {
  const supabase = getAdminSupabase();
  let repairStats = { repaired: 0, imageRepairs: 0, summaryRepairs: 0 };
  try {
    repairStats = await repairExistingGeneralRows(supabase);
  } catch (error) {
  ...
  if (!unique.length) return { found: 0, inserted: 0, skipped: 0, ...repairStats, errors, stats };
  ...
  if (!fresh.length) return { found: unique.length, inserted: 0, skipped: unique.length, ...repairStats, errors, stats };

  const enrichedRows = await Promise.all(fresh.map(enrichCandidate));
  const { data, error } = await supabase
    .from("hatsune_news")
    .insert(enrichedRows)
    .select("id,title,category,source_name,source_key,image_url,summary");
  if (error) throw error;
  ...
  return {
    ...
    ...repairStats,
    images: (data || []).filter((row) => row.image_url).length,
    summaries: (data || []).filter((row) => normalizeSpace(row.summary)).length,
    errors,
    stats,
    rows: data || [],
  };
