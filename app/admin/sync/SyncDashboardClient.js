...
export default function SyncDashboardClient() {
  ...
  async function syncRacers(){ setBusy(true); setMessage(""); const r=await fetch("/api/admin/sync/racers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({limit:20})}); const j=await r.json(); setMessage(r.ok||r.status===207?`選手マスタ同期: ${j.synced||0}人追加 / 合計${j.total_profiles||0}人${j.failed?.length?` / 失敗${j.failed.length}人`:""}`:j.error||"選手マスタ同期に失敗しました"); setBusy(false); }
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.panel}><div className={styles.panelTitle}><div><p>MANUAL CONTROL</p><h2>手動同期</h2></div><span>指示はSupabaseに登録され、Windowsエンジンが処理します。</span></div><div className={styles.commandGrid}>{COMMANDS.map(([key,label])=><button disabled={busy||r.state==="running"} key={key} onClick={()=>command(key)}>{label}</button>)}<button disabled={busy} onClick={syncRacers}>選手マスタ20人同期</button></div></section>
