...
const racerRegistration = value => String(value ?? '').trim().replace(/^0+(?=\d)/, '');
...
export function validateOfficialRowsAgainstRoster(source, storedEntries) {
  ...
  for (const row of rows) {
    ...
    const officialRacer = racerRegistration(row.racerNo);
    if (officialRacer && officialRacer !== racerRegistration(current.racer_registration_no)) {
...
export async function persistOfficialExhibition(client, race, source, options = {}) {
  ...
  for (const row of source.rows) {
    ...
    const storedEntry = storedEntries.find(entry => Number(entry.boat_no) === Number(row.boatNo));
    let query = client.from('bs_race_entries').update(update)
      .eq('race_date', race.race_date).eq('course_code', race.course_code).eq('race_no', race.race_no)
      .eq('boat_no', Number(row.boatNo));
    if (row.racerNo) query = query.eq('racer_registration_no', String(storedEntry.racer_registration_no));
