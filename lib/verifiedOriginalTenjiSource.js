...
import { fetchKiryuVerifiedOriginalTenji } from './kiryuVerifiedOriginalTenji.js';
...
export async function fetchBestOriginalTenji(race, options = {}) {
  if ([1,2,5,6,9,10,12,13,14,15,18,19,20,21,22,23,24].includes(Number(race?.courseCode))) {
    const fetchers = {
      1: fetchKiryuVerifiedOriginalTenji,
    ...
    return { ...official, sourceKind: 'official', fallbackUsed: false,
      diagnostics: { verifiedOfficial: { ok: official.ok, error: official.error || null, identity: official.identity || null,
        attempts: Array.isArray(official.attempts) ? official.attempts.map(({ status, error, mode }) => ({ status: status ?? null, error: error || null, mode: mode || null })) : [] } } };
  }
