...
function extractCandidates(html, source) {
  ...
  while ((match = anchorRegex.exec(html))) {
    ...
    // 分類はリンク周辺ではなく記事タイトルだけで判定する。
    // 隣の記事の「G1」「優勝」などを拾って誤分類するのを防ぐ。
...
async function repairExistingGeneralRows(supabase) {
  const { data, error } = await supabase
    .from("hatsune_news")
    .select("id,title,category,source_key,is_featured,priority")
  ...
  for (const row of data || []) {
    ...
    if (row.category === category && Boolean(row.is_featured) === isFeatured && Number(row.priority || 0) === priority) continue;

    const { error: updateError } = await supabase
      .from("hatsune_news")
      .update({ category, is_featured: isFeatured, priority })
      .eq("id", row.id);
