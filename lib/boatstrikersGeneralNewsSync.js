...
const MAX_IMAGE_REPAIR_PER_RUN = 8;
...
async function repairExistingGeneralRows(supabase) {
  ...
  for (const row of data || []) {
    ...
    if (!row.image_url && row.source_url && imageRepairs < MAX_IMAGE_REPAIR_PER_RUN) {
      try {
        const html = await fetchHtml(row.source_url);
        const imageUrl = extractArticleImage(html, row.source_url);
        if (imageUrl) {
          patch.image_url = imageUrl;
          imageRepairs += 1;
        }
      } catch {
        // 画像取得に失敗してもニュース同期自体は継続する。
      }
    }
  ...
  return { repaired, imageRepairs };
}

export async function syncBoatstrikersGeneralNews() {
  const supabase = getAdminSupabase();
  let repairStats = { repaired: 0, imageRepairs: 0 };
  try {
  ...
  const { data, error } = await supabase
    .from("hatsune_news")
    .insert(enriched)
    .select("id,title,category,source_name,source_key,image_url");
  if (error) throw error;
