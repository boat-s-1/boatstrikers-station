...
test('diagnostics include Kiryu and connected verified adapters',()=>assert.deepEqual(VENUES.filter(v=>v.endpoint).map(v=>v.code),[1,2,5,6,10,12,14,15,18,19,20,21,22,23,24]));
test('Kiryu is connected but half-lap is never labelled as one-lap',()=>{const venue=VENUES.find(v=>v.code===1);assert.equal(venue.stage,'接続済み');assert.match(venue.note,/半周を一周には使用しません/);});
