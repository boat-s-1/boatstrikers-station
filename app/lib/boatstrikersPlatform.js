...
export async function getRaceDetail(raceDate, courseCode, raceNo) {
  ...
  const baselineExhibition = toNumberOrNull(baseline?.avg_exhibition);

  const entries = entryRows.map((row) => {
    const entry = mapEntry(row);

    const venueDiffExhibition =
      entry.exhibition_time !== null && baselineExhibition !== null
        ? Number(
            (entry.exhibition_time - baselineExhibition).toFixed(2)
          )
        : null;
    ...
    return {
      ...
      venue_diff_exhibition: venueDiffExhibition,
    };
  });
