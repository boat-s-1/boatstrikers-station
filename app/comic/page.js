  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <nav className={styles.topNav} aria-label="コミックページ内メニュー">
          <a href="#top">TOP</a>
          <a href="#latest">最新話</a>
      ...
      <section className={styles.hero} id="top">
        <img
          src="/comic/minamo-main.jpeg"
          alt="私立みなも学園 ふなけん研究部 メインビジュアル"
      ...
      <nav className={styles.sectionNav} aria-label="コンテンツメニュー">
        ...
        <a href="#latest"><span>02</span>最新話</a>
        <a href="#characters"><span>03</span>登場人物</a>
      </nav>

      <section className={styles.introSection}>
        <p className={styles.sectionKicker}>BOAT RACE SCHOOL COMEDY</p>
        <h1>私立みなも学園<br /><span>〜ふなけん研究部〜</span></h1>
        <p>
          一果・キイナ・初音の3人が、学校の日常をボートレースに例えて大騒ぎ。
          部室や教室で巻き起こる、ゆるくて楽しいショートコメディです。
        </p>
      </section>
        ...
        {episodes.length > 0 ? (
              ...
              <article className={styles.episodeCard} key={episode.id}>
                <div className={styles.videoWrap}>
                  <iframe
                    ...
                    allowFullScreen
                  />
                  <span className={styles.episodeNumber}>NEW {index + 1}</span>
                </div>
                <div className={styles.cardBody}>
                  <h3>{episode.title}</h3>
                  {episode.publishedAt && (
                    <time dateTime={episode.publishedAt}>{formatDate(episode.publishedAt)} 公開</time>
                  )}
                </div>
              </article>
            ))}
          ...
          <div className={styles.fallbackPlayer}>
            <iframe
              ...
              allowFullScreen
            />
            <p>最新3話を取得できないため、再生リストを表示しています。</p>
          </div>
        )}

        <a href={playlistUrl} target="_blank" rel="noopener noreferrer" className={styles.youtubeButton}>
          YouTubeで全話を見る <span aria-hidden="true">›</span>
        </a>
      </section>

      <section className={styles.characterSection} id="characters">
        <div className={styles.characterHeading}>
          <span className={styles.sectionKicker}>CHARACTERS</span>
          <h2>登場人物紹介</h2>
          <p>アイコンをタップすると、キャラクター紹介が切り替わります。</p>
        </div>
        <CharacterGallery />
      </section>
