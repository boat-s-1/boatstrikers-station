...
const TABS = [
  { key: "all", label: "すべて" }, { key: "bs", label: "BS記事" }, { key: "women", label: "女子" }, { key: "grade", label: "SG・G1" },
  { key: "focus", label: "注目" }, { key: "result", label: "結果" }, { key: "racer", label: "選手" }, { key: "media", label: "動画", href: "/news/media" },
];
...
function featuredScore(item) {
  ...
  return recency + typeWeight + manualFeatured + priority + (item?.list_headline ? 4 : 0);
}
...
export default async function NewsTopPage({ searchParams }) {
  ...
  const afterFeatured = news.filter((i) => i.id !== featured?.id);
  const highlights = afterFeatured.slice(0, 3);
  const latest = afterFeatured.slice(3, 17);

  return <main className={styles.page}><div className={styles.shell}>
    <header className={styles.header}><div><span className={styles.eyebrow}>3 MINUTES NEWS</span><h1>BoatStrikers NEWS</h1><p>3分でボート界の「いま」が分かる。</p></div><Link href="/" className={styles.homeLink}>TOP</Link></header>
    ...
    <form className={styles.search} method="get"><input type="hidden" name="category" value={category} /><span>⌕</span><input name="q" defaultValue={q} placeholder="選手名・場名・キーワード" /><button type="submit">検索</button></form>

    {featured && <section className={styles.heroNews}><div className={styles.heroLabel}>TODAY'S TOP</div><Link href={`/news/${featured.id}`} className={styles.heroLink}><div className={styles.heroMeta}><span className={`${styles.categoryBadge} ${categoryClass(featured)}`}>{categoryLabel(featured)}</span><time>{formatHatsuneNewsDate(featured.published_at)}</time>{isNew(featured) && <span className={styles.newBadge}>NEW</span>}</div><h2>{headlineOf(featured)}</h2>{previewOf(featured) && <p>{previewOf(featured)}</p>}<b>30秒で読む →</b></Link></section>}

    {highlights.length > 0 && <section className={styles.highlights}><div className={styles.sectionHeading}><h2>注目ニュース</h2><span>まずはここから</span></div><div className={styles.highlightGrid}>{highlights.map((item) => <Link key={item.id} href={`/news/${item.id}`} className={styles.highlightCard}><div><span className={`${styles.categoryBadge} ${categoryClass(item)}`}>{categoryLabel(item)}</span><time>{shortDate(item.published_at)}</time></div><strong>{headlineOf(item)}</strong>{previewOf(item) && <p>{previewOf(item)}</p>}<b>読む →</b></Link>)}</div></section>}

    <section className={styles.latestSection}><div className={styles.sectionHeading}><h2>{category === "all" ? "最新ニュース" : TABS.find((x) => x.key === category)?.label}</h2><span>{news.length}件</span></div><div className={styles.headlineList}>{latest.map((item) => <Link key={item.id} href={`/news/${item.id}`} className={styles.headlineRow}><div className={styles.rowMeta}><time>{shortDate(item.published_at)}</time><span className={`${styles.categoryBadge} ${categoryClass(item)}`}>{categoryLabel(item)}</span></div><div className={styles.rowCopy}><strong>{headlineOf(item)}</strong>{previewOf(item) && <p>{previewOf(item)}</p>}</div><span className={styles.arrow}>›</span></Link>)}</div></section>

    {news.length === 0 && <div className={styles.empty}>該当するニュースはありません。</div>}

    <section className={styles.cta}><div><span>BOATSTRIKERS</span><strong>ニュースから今日のレースへ</strong><p>出走表・AI予想もそのまま確認できます。</p></div><Link href="/races">出走表を見る →</Link></section>
  </div></main>;
}
