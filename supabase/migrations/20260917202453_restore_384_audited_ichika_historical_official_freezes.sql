...
lock table public.bsc_official_predictions in share row exclusive mode;
lock table public.bsc_official_results in share row exclusive mode;

create temporary table _bsc_restore_manifest (
  id bigint primary key
) on commit drop;

insert into _bsc_restore_manifest (id) values
  (514582),
  (514584),
  (514586),
  (514678),
...
create temporary table _bsc_restore_state (
  pre_official_count bigint not null,
  pre_result_count bigint not null,
  pre_official_max_id bigint not null,
  restore_started_at timestamptz not null
) on commit drop;

insert into _bsc_restore_state
select
  (select count(*) from public.bsc_official_predictions),
  (select count(*) from public.bsc_official_results),
  coalesce((select max(id) from public.bsc_official_predictions), 0),
  clock_timestamp();
...
select
  ...
  coalesce(
    jsonb_agg(jsonb_build_object('ticket', ticket, 'score', 100000 - ord) order by ord),
    '[]'::jsonb
  ) as ticket_scores,
...
begin
  ...
  join public.bsc_official_predictions p
    on p.character_code=a.character_code
   and p.timing=a.timing
  ...
  then
    raise exception 'ID 615639 audit guard failed: predicted=%, deadline=%, margin=%, timing=%, published=%',
      v_predicted,v_deadline,v_margin,v_timing,v_published;
  end if;
end $$;
...
begin
  ...
  select
    ...
    jsonb_build_object(
      ...
      'ai_prediction_id',a.id,
      'displayed_bet_json',a.bet_json,
      'ticket_scores',t.ticket_scores,
      'bet_count',t.ticket_count,
      'bet_count_rule',a.detail_json->>'bet_count_rule',
  ...
  if v_inserted <> 384 then
    raise exception 'inserted row count mismatch: expected 384, got %',v_inserted;
  end if;
end $$;
...
declare
  v bigint;
  v_pre_official bigint;
  v_pre_results bigint;
  v_pre_max_id bigint;
begin
  select pre_official_count,pre_result_count,pre_official_max_id
  into v_pre_official,v_pre_results,v_pre_max_id
  from _bsc_restore_state;

  select count(*) into v
  from public.bsc_official_predictions p
  join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika';
  if v <> 384 then raise exception 'post official mismatch: expected 384, got %',v; end if;

  select count(*) into v
  from public.bsc_official_predictions p
  join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika' and p.timing='previous_day';
  if v <> 170 then raise exception 'post previous_day mismatch: expected 170, got %',v; end if;

  select count(*) into v
  from public.bsc_official_predictions p
  join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika' and p.timing='after_exhibition';
  ...
  from public.bsc_official_predictions p join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika';
  if v <> 1152 then raise exception 'post ticket mismatch: expected 1152, got %',v; end if;

  select count(*) into v
  from public.bsc_official_predictions p join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika' and p.unit_stake=100;
  if v <> 384 then raise exception 'unit stake mismatch: expected 384 rows at 100, got %',v; end if;

  select coalesce(sum(p.investment),0) into v
  from public.bsc_official_predictions p join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika';
  ...
  from public.bsc_official_predictions p join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions' and p.character_code='ichika'
    and p.investment <> jsonb_array_length(p.tickets)*p.unit_stake;
  ...
  from (
    ...
    select p.source_id from public.bsc_official_predictions p
      where p.source_table='bs_ai_predictions' and p.character_code='ichika'
  ) q;
  if v <> 0 then raise exception 'manifest minus official source mismatch: expected 0, got %',v; end if;

  select count(*) into v
  from (
    select p.source_id
    from public.bsc_official_predictions p
    where p.id>v_pre_max_id and p.source_table='bs_ai_predictions'
    except
    select id::text from _bsc_restore_manifest
  ) q;
  if v <> 0 then raise exception 'manifest-external newly inserted official rows: expected 0, got %',v; end if;
  ...
  where p.source_table='bs_ai_predictions'
    and p.snapshot->'displayed_bet_json' is distinct from a.bet_json;
  if v <> 0 then raise exception 'snapshot displayed_bet_json mismatch: expected 0, got %',v; end if;

  select count(*) into v
  from public.bsc_official_predictions p
  join _bsc_restore_manifest m on m.id::text=p.source_id
  join _bsc_restore_transformed t on t.id=m.id
  where p.source_table='bs_ai_predictions'
    and p.snapshot->'ticket_scores' is distinct from t.ticket_scores;
  ...
  join _bsc_restore_transformed t on t.id=m.id
  where p.source_table='bs_ai_predictions'
    and (p.snapshot->>'bet_count')::integer is distinct from t.ticket_count;
  if v <> 0 then raise exception 'bet_count mismatch: expected 0, got %',v; end if;

  select count(*) into v
  from (
    select character_code,timing,source_table,source_id,count(*) n
    from public.bsc_official_predictions p
    join _bsc_restore_manifest m on m.id::text=p.source_id
    where p.source_table='bs_ai_predictions'
    group by character_code,timing,source_table,source_id
    having count(*)>1
  ) q;
  if v <> 0 then raise exception 'duplicate source mismatch: expected 0, got %',v; end if;

  select count(*) into v from public.bsc_official_predictions;
  if v-v_pre_official <> 384 then raise exception 'global official count delta mismatch: expected +384, got %',v-v_pre_official; end if;

  select count(*) into v
  from public.bsc_official_predictions p
  where p.id>v_pre_max_id;
  if v <> 384 then raise exception 'new official ID range mismatch: expected 384, got %',v; end if;

  select count(*) into v from public.bsc_official_results;
  if v-v_pre_results <> 0 then raise exception 'bsc_official_results changed: expected delta 0, got %',v-v_pre_results; end if;

  select count(*) into v
  from public.bsc_official_results r
  join public.bsc_official_predictions p on p.id=r.prediction_id
  join _bsc_restore_manifest m on m.id::text=p.source_id
  where p.source_table='bs_ai_predictions';
