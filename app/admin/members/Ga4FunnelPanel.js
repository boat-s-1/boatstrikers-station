...
async function runReport(token,request){
  ...
  if(!response.ok){
    ...
    throw new Error(`GA4 Data API ${response.status}${text?`: ${text.slice(0,180)}`:""}`);
  }
  ...
  const [overview,events]=await Promise.all([
    ...
    runReport(token,{
      ...
      dimensionFilter:{
        filter:{
          fieldName:"eventName",
          inListFilter:{values:STAGES.filter(stage=>stage.event).map(stage=>stage.event)},
        },
      },
    }),
  ]);

  const map=new Map((events.rows||[]).map(row=>[
    row.dimensionValues?.[0]?.value,
    {users:metricValue(row,0),events:metricValue(row,1)},
  ]));
  const visitors=metricValue(overview.rows?.[0],0);
  const sessions=metricValue(overview.rows?.[0],1);
  const stages=STAGES.map(stage=>stage.event
    ?{...stage,users:map.get(stage.event)?.users||0,events:map.get(stage.event)?.events||0}
    :{...stage,users:visitors,events:sessions});
  return {visitors,sessions,stages};
}
...
async function loadGa4(){
  ...
  try{
    ...
    const [week,month]=await Promise.all([
      loadPeriod(token,"7daysAgo"),
      loadPeriod(token,"30daysAgo"),
    ]);
    return {configured:true,error:null,week,month};
  }catch(error){
    console.error("[GA4 funnel]",error);
    return {configured:true,error:error instanceof Error?error.message:"GA4データを取得できませんでした。"};
  }
}
...
function Funnel({data}){
  return <div className={styles.funnel}>
    {data.stages.map((stage,index)=>{
      const previous=index===0?null:data.stages[index-1];
      return <article key={stage.key} className={styles.stage}>
        <span>{stage.label}</span>
        <strong>{stage.users.toLocaleString("ja-JP")}</strong>
        <small>{index===0?`${data.sessions.toLocaleString("ja-JP")}セッション`: `前段階比 ${pct(stage.users,previous?.users||0)}`}</small>
      </article>;
    })}
  </div>;
}
  ...
  return <section className={styles.section}>
    <div className={styles.heading}>
      <div>
        ...
        <h2>集客 → 会員化ファネル</h2>
        <p>訪問からAI利用、会員登録、LINE・Discord、PREMIUM導線までをGA4のユニークユーザー数で追います。</p>
      </div>
      <div className={styles.measurement}>G-DXF6FFZ574</div>
    </div>

    {!data.configured&&<div className={styles.notice}>
      <strong>GA4読み取り設定待ち</strong>
      <p>画面は実装済みです。Vercelに <code>GA4_CLIENT_EMAIL</code> と <code>GA4_PRIVATE_KEY</code> を設定すると自動で実データ表示に切り替わります。Property ID は {PROPERTY_ID} を使用します。</p>
    </div>}

    {data.error&&<div className={styles.error}>GA4取得エラー: {data.error}</div>}

    {data.week&&<>
      <div className={styles.periodTitle}><strong>直近7日</strong><span>GA4 unique users</span></div>
      <Funnel data={data.week}/>
      <div className={styles.periodTitle}><strong>直近30日</strong><span>GA4 unique users</span></div>
      <Funnel data={data.month}/>
      <p className={styles.note}>※ 各段階はGA4イベント到達ユーザー数です。厳密な同一ユーザー順序のコホートではなく、改善判断用の到達率として表示しています。β期間中は「PREMIUM導線」をCTA到達として表示します。</p>
    </>}
  </section>;
}
