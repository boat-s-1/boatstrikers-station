...
function EmptyAi({ type }) {
  return (
    <div className={styles.emptyAi}>
      ...
      <h3>
        {type === "live" ? "一果の直前版" : "一果の前日版"}
        は準備中です
      </h3>
      <p>
        Phase 3.2でIchika Brainを接続すると、
        学習モデルの期待度・印・コメント・買い目が表示されます。
      </p>
    </div>
  );
}
...
function PredictionPanel({ prediction, title }) {
  if (!prediction) {
    return (
      <EmptyAi type={title.includes("直前") ? "live" : "previous"} />
    );
  }
...
export default function RaceDetailTabs({
  ...
  const exhibitionRows = useMemo(() => {
    ...
    return entries.map((entry) => ({
      ...entry,
      exhibitionRank:
        timeRank.get(entry.boat_no) || null,
    }));
  }, [entries]);
