...
function parseBetCombination(value) {
  const raw =
    typeof value === "string"
      ? value
      : value?.bet ?? value?.combination ?? value?.ticket ?? "";
...
function buildBoatPriority(prediction) {
  const bets = Array.isArray(prediction?.bet_json)
    ? prediction.bet_json
    : [];

  const betBoats = bets.flatMap(
    (bet) => parseBetCombination(bet).numbers
  );

  const markBoats = normalizeMarks(prediction).map(
    (mark) => Number(mark.boatNo)
  );
...
function uniqueTickets(tickets = []) {
  return tickets.filter(
    (ticket, index, array) =>
      ticket &&
      array.indexOf(ticket) === index
  );
}

function buildBetModes(prediction) {
  const priority = buildBoatPriority(prediction);

  const mainBoat =
    Number(prediction?.main_boat) ||
    priority[0] ||
    1;

  const opponents = priority.filter(
    (boatNo) => boatNo !== mainBoat
  );

  const secondBoat = opponents[0] || 2;
  const thirdBoat = opponents[1] || 3;
  const fourthBoat = opponents[2] || 4;

  const rawBets = Array.isArray(prediction?.bet_json)
    ? prediction.bet_json
    : [];

  const exactBets = uniqueTickets(
    rawBets
      .map((bet) => parseBetCombination(bet).raw)
      .filter((bet) => /^\s*[1-6]\s*[-－]\s*[1-6]\s*[-－]\s*[1-6]\s*$/.test(bet))
      .map((bet) => bet.replace(/[－\s]/g, "-"))
  );

  const oni = uniqueTickets([
    exactBets[0],
    exactBets[1],
    `${mainBoat}-${secondBoat}-${thirdBoat}`,
    `${mainBoat}-${thirdBoat}-${secondBoat}`,
  ]).slice(0, 2);

  const hit = uniqueTickets([
    `${mainBoat}-${secondBoat}-全`,
    `${mainBoat}-${thirdBoat}-全`,
  ]);

  const recovery = uniqueTickets([
    ...exactBets,
    `${mainBoat}-${secondBoat}-${thirdBoat}`,
    `${mainBoat}-${thirdBoat}-${secondBoat}`,
    `${mainBoat}-${secondBoat}-${fourthBoat}`,
    `${mainBoat}-${fourthBoat}-${secondBoat}`,
    `${mainBoat}-${thirdBoat}-${fourthBoat}`,
    `${mainBoat}-${fourthBoat}-${thirdBoat}`,
  ]).slice(0, 6);
  ...
  const holeHead =
    ...
    (mainBoat === 1 ? thirdBoat : opponents[0]) ||
    3;

  const holeOpponents = uniqueNumbers([
    secondBoat,
    thirdBoat,
    mainBoat,
    fourthBoat,
  ]).filter((boatNo) => boatNo !== holeHead);

  const hole = uniqueTickets([
    `${holeHead}-${holeOpponents[0] || mainBoat}-全`,
    `${holeHead}-${holeOpponents[1] || secondBoat}-全`,
  ]);

  const score = finiteNumber(
    prediction?.score ??
      prediction?.escape_probability ??
      prediction?.confidence
  );

  let recommendedMode = "的中率";
  let recommendationReason =
    "現段階では点数を少し広げ、相手抜けを抑える構成です。";

  if (score !== null && score >= 80) {
    recommendedMode = "鬼絞り";
    recommendationReason =
      "AI評価が高いため、上位2点へ絞る構成を推奨します。";
  } else if (score !== null && score < 55) {
    recommendedMode = "穴狙い";
    recommendationReason =
      "本命信頼度が低いため、別艇の1着から高配当を狙う構成です。";
  } else if (exactBets.length >= 4) {
    recommendedMode = "回収率";
    recommendationReason =
      "AI買い目の候補が複数あるため、組み合わせを選別して回収率を狙います。";
  }
  ...
  return {
    ...
    modes: [
      ...
      },
      {
        ...
        subtitle: "本線と中穴を選別",
        tickets: recovery,
        note: "AI買い目を優先しつつ、点数を広げすぎない構成です。",
        accent: "#16834d",
        background: "#f2fbf6",
        border: "#bee7cf",
      },
...
function normalizeResultCombination(value) {
  if (value === null || value === undefined || value === "") return null;
  ...
  return Array.isArray(numbers) && numbers.length === 3
    ? numbers
    : null;
}

function getActualTrifecta(result, resultEntries = [], event = null) {
  const directCandidates = [
    result?.trifecta,
    result?.trifecta_result,
    result?.sanrentan,
    result?.combination,
    result?.result_combination,
    event?.trifecta,
    event?.trifecta_result,
    event?.sanrentan,
  ];

  for (const candidate of directCandidates) {
    const normalized = normalizeResultCombination(candidate);
    if (normalized) return normalized;
  }

  const rankedEntries = (Array.isArray(resultEntries) ? resultEntries : [])
    .map((entry) => ({
      boatNo: Number(
        entry?.boat_no ??
          entry?.teiban ??
    ...
    .filter(
      (entry) =>
        Number.isInteger(entry.boatNo) &&
        entry.boatNo >= 1 &&
        entry.boatNo <= 6 &&
        Number.isInteger(entry.arrivalOrder) &&
        entry.arrivalOrder >= 1 &&
        entry.arrivalOrder <= 3
    )
    .sort((a, b) => a.arrivalOrder - b.arrivalOrder);

  if (rankedEntries.length >= 3) {
    return rankedEntries.slice(0, 3).map((entry) => entry.boatNo);
  }

  return null;
}

function ticketMatchesResult(ticket, actualTrifecta) {
  if (!Array.isArray(actualTrifecta) || actualTrifecta.length !== 3) {
    return false;
  }
  ...
  return parts.every((part, index) => {
    if (part === "全" || part.toLowerCase() === "all" || part === "*") {
      return true;
    }

    return Number(part) === Number(actualTrifecta[index]);
  });
}

function formatActualTrifecta(actualTrifecta) {
  return Array.isArray(actualTrifecta) && actualTrifecta.length === 3
    ? actualTrifecta.join("-")
    : null;
}
...
function getTrifectaPayout(result, event = null) {
  const value =
    result?.trifecta_payout ??
    result?.sanrentan_payout ??
    result?.payout_trifecta ??
    event?.trifecta_payout ??
    event?.sanrentan_payout ??
    null;
...
function calculateModeSettlement(
  mode,
  actualTrifecta,
  trifectaPayout,
  unitStake = 100
  ...
  const recoveryRate =
    investment > 0
      ? Math.round((payout / investment) * 1000) / 10
      : 0;
...
function formatSignedYen(value) {
  ...
  if (!Number.isFinite(number)) return "-";
  const sign = number > 0 ? "+" : "";
  return `${sign}${Math.round(number).toLocaleString("ja-JP")}円`;
}

function BetModeCard({ mode, recommended, actualTrifecta, trifectaPayout, unitStake = 100 }) {
  const resultSynced =
    Array.isArray(actualTrifecta) && actualTrifecta.length === 3;

  const hitTickets = resultSynced
    ? mode.tickets.filter((ticket) =>
        ticketMatchesResult(ticket, actualTrifecta)
      )
    : [];

  const modeHit = hitTickets.length > 0;
  const settlement = calculateModeSettlement(
    mode,
    actualTrifecta,
    trifectaPayout,
    unitStake
  ...
  return (
    <section
      style={{
        ...
        overflow: "hidden",
        background: "#fff",
        boxShadow: "0 8px 22px rgba(30, 55, 85, .07)",
      }}
    >
      <div
        style={{
          ...
          {resultSynced && (
            <span
              style={{
                ...
                padding: "4px 8px",
                borderRadius: "999px",
                background: modeHit ? "#14945a" : "#e7edf3",
                color: modeHit ? "#fff" : "#6b7888",
                fontSize: "10px",
                fontWeight: 1000,
              }}
            >
      ...
      >
        {mode.tickets.map((ticket, index) => {
          ...
          return (
            <div
              key={`${mode.key}-${ticket}-${index}`}
              style={{
                padding: "12px 14px",
                borderRadius: "12px",
                background: isHit ? "#e9fbf1" : "#f6f9fc",
                border: isHit
                  ? "1px solid #8fd5ad"
                  : "1px solid transparent",
                color: isHit ? "#126a3f" : "#263d55",
                fontSize: "19px",
                fontWeight: 1000,
                letterSpacing: ".04em",
                display: "flex",
        ...
        {resultSynced && (
          ...
          >
            {[
              ["投資額", formatYen(settlement.investment)],
              ["払戻", formatYen(settlement.payout)],
              ["利益", formatSignedYen(settlement.profit)],
              [
                "回収率",
                `${settlement.recoveryRate.toLocaleString(
                  "ja-JP"
                )}%`,
              ],
            ].map(([label, value]) => (
...
function MultiModeBetPanel({ prediction, result, resultEntries, event }) {
  const betModes = buildBetModes(prediction);
  const actualTrifecta = getActualTrifecta(
    result,
    resultEntries,
    event
  );
  const actualCombination = formatActualTrifecta(actualTrifecta);
  const trifectaPayout = getTrifectaPayout(result, event);
  const unitStake = 100;
  ...
  return (
    <>
      <div className={styles.panelHeading}>
        <div>
          <p>AI BET NAVIGATION</p>
          <h2>一果の買い方ナビ</h2>
        </div>

        <span className={styles.panelBadge}>4モード</span>
      </div>
      ...
      {actualCombination && (
        <div
          style={{
            ...
            background:
              "linear-gradient(135deg, #f2fff8 0%, #e7faef 100%)",
            color: "#176640",
          }}
        >
          <span
          ...
          >
            各買い方と買い目に、的中・不的中を自動表示しています。
            {Number.isFinite(trifectaPayout)
              ? ` 三連単払戻は100円あたり${trifectaPayout.toLocaleString(
                  "ja-JP"
                )}円です。`
              : ""}
          </p>
      ...
      >
        {betModes.modes.map((mode) => (
          <BetModeCard
            key={mode.key}
            mode={mode}
            recommended={betModes.recommendedMode === mode.title}
            actualTrifecta={actualTrifecta}
            trifectaPayout={trifectaPayout}
            unitStake={unitStake}
          />
        ))}
      </div>
      ...
      >
        現在はAIの本命艇・印・既存買い目を利用した暫定ルール版です。
        「全」は残りの艇への流しを表します。オッズと着順確率を連携すると、
        回収率モードと穴狙いを期待値ベースへ発展できます。
      </div>
    </>
  );
}

function BetPanel({ prediction, noteFeature, result, resultEntries, event }) {
  if (!prediction) {
    return (
      <div className={styles.emptyAi}>
        <div className={styles.emptyAiIcon}>🎯</div>
  ...
  if (
    hasPublishedNote(noteFeature, "live") ||
    hasPublishedNote(noteFeature, "previous")
  ) {
    return (
      <>
        <MarksPanel prediction={prediction} title="最終印" />
