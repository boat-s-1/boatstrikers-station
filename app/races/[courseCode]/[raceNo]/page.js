...
export default async function RaceDetailPage({ params, searchParams }) {
  ...
  try {
    ...
    console.error(error);
    loadError = error instanceof Error ? error.message : "出走表の読み込みに失敗しました。";
  }

  const courseName = getCourseName(courseCode);
  const paddedCourseCode = String(courseCode).padStart(2, "0");
    ...
    <main className={styles.page}>
      ...
      <section className={styles.compactRaceHeader}>
        ...
        <div className={styles.compactRaceMeta}>
          <span>{raceDate}</span>
          <span>同期 {formatJstDateTime(data?.event?.synced_at)}</span>
        </div>
      </section>
      ...
      <section className={styles.content}>
        ...
        ) : !data?.event || !Array.isArray(data?.entries) || data.entries.length === 0 ? (
          <div className={styles.messageCard}>このレースの出走表はありません。</div>
        ) : (
          <>
            <RaceDetailTabs
              event={data.event}
            ...
            <nav className={styles.moveNav}>
              {raceNo > 1 ? (
                <Link
                  href={`/races/${paddedCourseCode}/${raceNo - 1}?date=${raceDate}`}
                  prefetch={false}
                >
                  ← {raceNo - 1}R
                </Link>
