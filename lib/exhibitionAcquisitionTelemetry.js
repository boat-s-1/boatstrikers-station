...
export const REASON_LABELS = {
  ready: '取得・6艇保存成功', not_published: '展示公開待ち',
...
const GROUPS = {
  ...
  identity_mismatch: ['source_venue_mismatch','reference_identity_mismatch','original_race_mismatch','source_identity_mismatch','source_race_mismatch','boat_identity_mismatch','original_boat_identity_invalid','reference_boat_identity_invalid','roster_or_exhibition_mismatch','saved_roster_mismatch','entry_update_identity_mismatch'],
  identity_missing: ['reference_identity_missing','source_identity_missing'],
  incomplete_boats: ['six_unique_boats_required','saved_roster_incomplete','incomplete_persistence'],
  ...
  unavailable: ['official_original_tenji_not_available','official_measurements_unavailable','original_data_unavailable','detail_not_available'],
  not_published: ['not_published'],
};
const SOURCE_KEYS = ['boaters','tsu','amagasaki','official','verifiedOfficial','pcKyotei'];
...
export function acquisitionRecord(race, consumer, result, startedAt, checkedAt) {
  const sourceKind = ['official','boaters','pc_kyotei','mixed'].includes(result?.sourceKind) ? result.sourceKind : 'unknown';
...
export async function recordExhibitionAcquisition(client, consumer, race, result, logger = console) {
  const now = new Date().toISOString();
  try {
    const payload = acquisitionRecord(race, consumer, result, now, now);
    ...
    logger.warn('exhibition_acquisition_record_failed');
    return false;
  }
}

export async function trackExhibitionFetch(client, consumer, race, fetcher, options = {}, logger = console, finalize = null) {
  ...
  let result;
  try {
    result = await fetcher(race, options);
    // A production acquisition is only ready after all six stored rows have been
    // updated.  Recording before this hook caused fetch-success/save-failure
    // races to be shown as ready in the admin screen.
    if (typeof finalize === 'function') result = await finalize(result);
  }
