...
export async function autoPublishGradeAndRacerNews({ limit = 6, scanLimit = 40 } = {}) {
  ...
  const { data: candidates, error: readError } = await client
    ...
    .select("id,collected_at,published_at,title,category,summary,source_name,source_url,importance,target_character,status,raw_payload,verified,verification_status,result_status,verification_note")
  ...
  for (const { item, publicCategory } of eligible) {
    try {
      ...
      const rawPayload = {
        ...(item.raw_payload || {}),
        editorial_materials: materials,
        auto_published_at: now,
        auto_public_category: publicCategory,
        verified_before_publish: true,
        verification_note: item.verification_note || null,
        result_status_at_publish: item.result_status || "unknown",
      };
