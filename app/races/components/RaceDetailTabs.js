...
function MetricCard({ label, value, accent = "#1769e0" }) {
  return (
    <div style={{ padding: "13px", border: "1px solid #e1e8f1", borderRadius: "14px", background: "#fff" }}>
      <span style={{ display: "block", color: "#748398", fontSize: "11px", fontWeight: 900 }}>{label}</span>
      <strong style={{ display: "block", marginTop: "5px", color: accent, fontSize: "20px" }}>{value ?? "-"}</strong>
    </div>
  );
}
...
function IchikaPreviousPanel({ prediction, entries, noteFeature }) {
  ...
  const escapeScore = factorValue(prediction, "escape_probability", "in_escape_score", "score");
  const motorEvaluation = factorValue(prediction, "motor_evaluation", "motor_grade", "motor_score");
  const stEvaluation = factorValue(prediction, "st_evaluation", "st_grade", "st_score");
  const localEvaluation = factorValue(prediction, "local_evaluation", "local_grade", "local_score");
  const similarCount = factorValue(prediction, "similar_race_count", "similar_count", "sample_count");
  const pastPerformance = factorValue(prediction, "past_performance", "historical_result", "historical_hit_rate");
  ...
  return (
    <div className={styles.predictionPanel}>
      <div className={styles.predictionHero} style={{ background: "linear-gradient(135deg, #fff7eb 0%, #ffe0b4 100%)" }}>
        <div>
          <span className={styles.aiEyebrow}>ICHIKA PRE-DAY AI</span>
          <h3>一果AI 前日版</h3>
          <small>選手・モーター・コース実績を中心に分析</small>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "10px", marginTop: "14px" }}>
        <MetricCard label="AI期待度" value={displayPercent(prediction.score)} accent="#e16318" />
        <MetricCard label="イン逃げ期待度" value={displayPercent(escapeScore)} accent="#e16318" />
        <MetricCard label="AI信頼度" value={prediction.rank || displayEvaluation(prediction.score)} accent="#b13e00" />
        <MetricCard label="類似レース件数" value={similarCount === null ? "-" : `${similarCount}件`} accent="#6c4a2a" />
        <MetricCard label="モーター評価" value={displayEvaluation(motorEvaluation)} />
        <MetricCard label="ST評価" value={displayEvaluation(stEvaluation)} />
        <MetricCard label="当地評価" value={displayEvaluation(localEvaluation)} />
        <MetricCard label="過去成績" value={pastPerformance ?? "-"} />
      </div>
