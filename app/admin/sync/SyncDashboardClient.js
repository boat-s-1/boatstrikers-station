...
export default function SyncDashboardClient() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.panel}><div className={styles.panelTitle}><div><p>MANUAL CONTROL</p><h2>手動同期</h2></div><span>指示はSupabaseに登録され、Windowsエンジンが処理します。</span></div><div className={styles.commandGrid}>{COMMANDS.map(([key,label])=><button disabled={busy||r.state==="running"} key={key} onClick={()=>command(key)}>{label}</button>)}<button disabled={busy} onClick={syncRacers}>選手マスタ50人同期</button></div></section>
