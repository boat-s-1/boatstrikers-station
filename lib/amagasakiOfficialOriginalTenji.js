...
export async function fetchAmagasakiOfficialOriginalTenji(race, options = {}) {
  ...
  try {
    ...
    const identityOk = selected.length === 1
      && new RegExp(`data-day=(['"])${raceDate}\\1`, "i").test(selected[0][0])
      && new RegExp(`data-race=(['"])${raceNo}\\1`, "i").test(selected[0][0])
      && /data-req=(['"])cyokuzen\1/i.test(selected[0][0]);
    if (!identityOk) {
