...
const ALIASES=Object.fromEntries(STADIUMS.flatMap(x=>[[x.name,x.slug],[x.slug,x.slug]]));
...
export async function getStadiumAiV2(value){
  const stadium=resolveStadium(value);const sb=supabase();
  if(!sb)return {stadium,payload:null,error:'Supabase環境変数が未設定です。'};
  const {data,error}=await sb.from('stadium_data_snapshots').select('*').eq('course_code',stadium.courseCode).maybeSingle();
  return {stadium,payload:data?.payload||null,generatedAt:data?.generated_at||null,error:error?.message||null};
}
