...
const SERIES = { ichika: { label: "一果", name: "イン逃げ鉄板ゼミ" }, hatsune: { label: "初音", name: "女子戦攻略マガジン" }, kiina: { label: "キイナ", name: "穴党新聞" } };
const empty = { id: null, series: "ichika", issue_no: "002", number_label: "第002号", title: "", summary: "", premium_start_page: 5, status: "draft", published_at: "", page_paths: [] };
...
export default function SeminarMagazineAdminClient(){
  const [issue,setIssue]=useState(empty),[issues,setIssues]=useState([]),[busy,setBusy]=useState(false),[message,setMessage]=useState("");
  ...
  function fresh(){ setIssue({...empty}); setMessage(""); window.scrollTo({top:0,behavior:'smooth'}); }
  ...
  return <>
    ...
    <section className={styles.panel}>
      ...
      <div className={styles.grid3}>
        <label>シリーズ<select value={issue.series} onChange={e=>field('series',e.target.value)}>{Object.entries(SERIES).map(([k,v])=><option key={k} value={k}>{v.label}｜{v.name}</option>)}</select></label>
      ...
      <div className={styles.grid3}>
        ...
        <label>公開日時<input type="datetime-local" value={issue.published_at||''} onChange={e=>field('published_at',e.target.value)} /></label>
    ...
    <section className={styles.list}><h2>登録済み</h2>{issues.length===0?<p>まだ管理画面から登録した号はありません。</p>:issues.map(x=><article key={x.id}><div><small>{SERIES[x.series]?.label}・{x.number_label||x.issue_no}</small><h3>{x.title}</h3><p>{x.status==='published'?'公開中':'下書き'} / Premium {x.premium_start_page}P〜 / {(x.page_paths||[]).length}P</p></div><button onClick={()=>edit(x)}>編集</button></article>)}</section>
  </>;
}
