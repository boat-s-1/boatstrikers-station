...
export default function MinamoComicPromptBuilder({ payload, newsItems = [] }) {
  ...
  }, [payload, topic, news, selectedCandidate]);
    ...
    <section id="minamo-comic" style={{ scrollMarginTop: 80, marginTop: 28, padding: 20, borderRadius: 18, border: "1px solid rgba(255,255,255,.12)", background: "rgba(7,18,38,.72)" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: 0, opacity: .78, lineHeight: 1.7 }}>前日の実結果とBoatStrikers NEWSから4コマ候補を3本出し、選ぶだけで生成プロンプトへ反映します。</p>
        </div>
      ...
      <div style={{ marginTop: 20 }}>
        <div style={{ fontSize: 13, opacity: .75, marginBottom: 9 }}>今日の4コマ候補 3本</div>
            ...
            return (
              <article key={candidate.id} style={{ border: active ? "2px solid #f2c75c" : "1px solid rgba(255,255,255,.12)", borderRadius: 14, padding: 14, background: active ? "rgba(242,199,92,.08)" : "rgba(255,255,255,.035)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ fontSize: 11, opacity: .58 }}>優先度 {candidate.priority}</span>
                </div>
                <h3 style={{ fontSize: 17, margin: "10px 0 8px" }}>{candidate.title}</h3>
                <p style={{ fontSize: 13, lineHeight: 1.65, opacity: .78, minHeight: 64 }}>{candidate.lead}</p>
                <div style={{ display: "grid", gap: 4, marginTop: 10 }}>
                  {candidate.facts.slice(0, 4).map((f) => <div key={`${candidate.id}-${f.label}`} style={{ fontSize: 12, opacity: .72 }}>・{f.label}：{f.value}</div>)}
                </div>
                <div style={{ fontSize: 12, marginTop: 10, opacity: .72 }}>主役：{candidate.recommendedCharacters.map((c) => CHARACTER_LABELS[c] || c).join(" → ")}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(180px, 280px) 1fr", gap: 14, marginTop: 18 }}>
        <label style={{ display: "grid", gap: 7 }}>
          ...
          <select value={topic} onChange={(e) => { setTopic(e.target.value); setSelectedCandidate(null); }} style={{ width: "100%", padding: 12, borderRadius: 10 }}>
        ...
        <label style={{ display: "grid", gap: 7 }}>
          ...
          <textarea value={news} onChange={(e) => { setNews(e.target.value); setSelectedCandidate(null); }} placeholder="例：徳山G1初日、○○選手が… ※確認済みの事実だけ入力" style={{ width: "100%", minHeight: 88, padding: 12, borderRadius: 10, resize: "vertical" }} />
        </label>
      </div>

      <div style={{ marginTop: 16 }}>
        <div style={{ fontSize: 13, opacity: .75, marginBottom: 7 }}>生成プロンプト</div>
        <textarea readOnly value={prompt} style={{ width: "100%", minHeight: 560, padding: 14, borderRadius: 12, resize: "vertical", lineHeight: 1.65, fontFamily: "inherit" }} />
      </div>
    </section>
  );
}
