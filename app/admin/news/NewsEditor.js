    ...
    <div className={styles.workspace}>
      ...
      <aside className={styles.previewPanel}>
        ...
        <div className={styles.localBox}>
          ...
          {job && <div className={styles.progress}><div><span style={{ width: `${job.progress || 0}%` }} /></div><p>{job.message}</p>{job.status === "error" && job.errorDetails && <pre className={errorStyles.errorDetails}>{job.errorDetails}</pre>}{job.status === "error" && job.logPath && <p className={errorStyles.logPath}>詳細ログ：{job.logPath}</p>}{job.status === "complete" && <button type="button" onClick={openOutput}>保存フォルダを開く</button>}</div>}
