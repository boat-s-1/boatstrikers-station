...
function absoluteOfficialUrl(href) {
  try {
    return new URL(String(href || "").replace(/&amp;/g, "&"), OFFICIAL_NEWS_URL).toString();
  } catch {
    return OFFICIAL_NEWS_URL;
  }
}

function parseOfficialDate(rawTitle, context = "") {
  const source = `${rawTitle || ""} ${context || ""}`;
  const match = source.match(/(20\d{2})[\/.\-](\d{1,2})[\/.\-](\d{1,2})/);
  if (!match) return null;
  return `${match[1]}-${String(match[2]).padStart(2, "0")}-${String(match[3]).padStart(2, "0")}`;
}

function cleanOfficialTitle(rawTitle) {
...
async function collectOfficialNews() {
  ...
  const candidates = [];
  const seen = new Set();
  const allowedDates = new Set([jstDate(0), jstDate(-1)]);
  const anchorRegex = /<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;
  let match;

  while ((match = anchorRegex.exec(html))) {
    const rawTitle = normalizeSpace(decodeHtml(match[2]));
    const before = html.slice(Math.max(0, match.index - 300), match.index);
    const date = parseOfficialDate(rawTitle, before);
    if (!date || !allowedDates.has(date)) continue;

    const title = cleanOfficialTitle(rawTitle);
    if (title.length < 8 || !WOMEN_KEYWORDS.some((keyword) => title.includes(keyword))) continue;
    ...
    candidates.push({
      title,
      summary: `BOAT RACEオフィシャルウェブサイトで${date}に公開された女子ボートレース関連のお知らせです。初音NEWSでは要点を整理して紹介します。`,
...
async function collectRaceData(supabase) {
  ...
  for (const [courseCode, rows] of todayByCourse) {
    ...
    if (firstDay) {
      ...
      if (top) {
        const racerName = normalizeSpace(top.name);
        candidates.push({
          title: `${place}女子戦の高モーターは${racerName}の${top.motorNo}号機・2連対率${top.rate.toFixed(1)}%`,
          summary: `${place}の女子戦初日データから、出場選手のモーター2連対率を比較。現時点のトップは${racerName}選手の${top.motorNo}号機で${top.rate.toFixed(1)}%です。`,
          category: "motor",
          source_type: "bs_data",
          source_name: "BoatStrikers レースDB",
