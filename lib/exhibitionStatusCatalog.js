...
export const VENUES = names.map((name,i)=>{
 const code=i+1, verified=[5,14,15,18,23].includes(code), staging=[12,24].includes(code), legacy=[7,9,11,13,17].includes(code);
 return {code,name,stage:verified?'接続済み':staging?'検証用実装':legacy?'既存取得経路':'調査中',
  ...
  note:code===24?'進入コースと艇番の照合待ち。通知未接続。':code===18?'HTTPS公式出走情報と6艇の登録番号・展示タイムを照合。直線なし。':code===12?'開催中の更新確認待ち。通知未接続。':code===23?'当日限定。翌日ページへの切替に注意。':code===5?'公式出走情報と6艇を照合。':verified?'開催日・レース・艇番を照合。':legacy?'既存の取得処理あり。この画面の個別診断は未接続。':'取得先・照合方法を調査中。',
 };
});
