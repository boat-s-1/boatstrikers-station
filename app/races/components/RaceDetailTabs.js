...
function BetModeCard({ mode, recommended }) {
  return (
    <section
      style={{
...
function MultiModeBetPanel({ prediction }) {
  const result = buildBetModes(prediction);
      ...
      >
        {result.modes.map((mode) => (
          <BetModeCard
            key={mode.key}
            mode={mode}
            recommended={result.recommendedMode === mode.title}
          />
        ))}
      </div>
...
function BetPanel({ prediction, noteFeature }) {
  ...
  return <MultiModeBetPanel prediction={prediction} />;
}
