...
import { VENUES, INVENTORY_DATE, STAGES, LIVE_STATES, diagnosticStatus } from '../../../../lib/exhibitionStatusCatalog';
...
export default function ExhibitionStatus(){
 ...
 const shown=VENUES.filter(v=>filter==='all'||(filter==='checkable'?v.endpoint:v.stage===filter||v.operationalState===filter));
 return <main className={styles.page}><div className={styles.shell}>
  ...
  <div className={styles.summary}>{LIVE_STATES.map(state=><span key={state}>{state} <strong>{VENUES.filter(v=>v.operationalState===state).length}場</strong></span>)}</div>
  ...
  <div className={styles.controls}>
   ...
   <label>絞り込み<select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">全24場</option><option value="checkable">個別確認できる場</option>{LIVE_STATES.map(s=><option key={s}>{s}</option>)}{STAGES.filter(s=>!LIVE_STATES.includes(s)).map(s=><option key={s}>{s}</option>)}</select></label>
