...
async function buildCharacterDetails(client, raceDate, digest) {
  const [{ data: events, error: eventError }, { data: entries, error: entryError }] = await Promise.all([
    ...
    client.from("bs_race_entries")
      .select("course_code,race_no,boat_no,arrival_order")
      .eq("race_date", raceDate)
  ...
  for (const row of entries || []) {
    if (row.arrival_order === 1) winnerMap.set(`${row.course_code}-${row.race_no}`, Number(row.boat_no));
  }
  ...
  const womenEvents = validEvents.filter(isWomenRace);
  ...
  const hatsune = {
    ...
    detection_note: "女子戦判定はレース名に『オールレディース』『ヴィーナス』『女子』『レディース』を含むレースを対象。",
    comment: "",
  };
  const kiina = {
...
function buildOutputs(digest, characterDetails) {
  ...
  const imagePayload = {
    template: "data_lab_yesterday_numbers_v2",
