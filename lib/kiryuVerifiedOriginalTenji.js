...
export function parseKiryuOriginalTenji(ajaxHtml) {
  ...
  for (const match of section.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)) {
    ...
    const halfLapTime = numeric(cell(row, 'col5_1'), 10, 25);
    const turnTime = numeric(cell(row, 'col5_2'), 2, 20);
    const straightTime = numeric(cell(row, 'col5_3'), 2, 20);
...
async function text(url, options, timeoutMs) {
  const controller = new AbortController(), timer = setTimeout(()=>controller.abort(),timeoutMs);
  try {
    const response = await fetch(url,{cache:'no-store',redirect:'manual',signal:controller.signal,headers:{'user-agent':'BoatStrikers/1.0 (+https://www.boat-strike.online/)','x-requested-with':'XMLHttpRequest',referer:'https://www.kiryu-kyotei.com/'}});
...
export async function fetchKiryuVerifiedOriginalTenji(race, options={}) {
  ...
  const [page,ajax,reference]=await Promise.all([text(pageUrl,options,timeoutMs),text(ajaxUrl,options,timeoutMs),fetchNationalBeforeInfo(race,{timeoutMs})]);
  if (!page.ok || !ajax.ok) return {...fail(page.error||ajax.error),pageUrl,ajaxUrl};
