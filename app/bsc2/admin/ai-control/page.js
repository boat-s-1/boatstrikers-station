...
const JOB_TYPES = {
  ...
  after_exhibition_prediction: {
    title: "展示後版",
    description: "展示後CSVから直前予測と診断を更新します。",
    icon: "🚤",
    dateRequired: true,
  },
  full_pipeline: {
    title: "一括処理",
    description: "取込・予測・補正・診断を順番に実行します。",
    icon: "⚡",
    dateRequired: true,
  },
};
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      if (response.status === 413) {
        throw new Error(
          "CSVがVercel経由の上限を超えています。Supabase直送APIの配置を確認してください。",
        );
      }
      throw new Error(
        `${fallbackMessage}（HTTP ${response.status}）: ${text.slice(0, 180)}`,
      );
    }
    throw error;
  }
...
function UploadCard({
  ...
  async function upload(event) {
    ...
    try {
      ...
      const urlBody = await readJsonResponse(
        urlResponse,
        "アップロード準備に失敗しました",
      );
      ...
      const finalizeBody = await readJsonResponse(
        finalizeResponse,
        "ジョブ登録に失敗しました",
      );

      setMessage(
        `登録完了：ジョブ ${finalizeBody.job.id.slice(0, 8)}`,
      );
      setFile(null);
        ...
        <div className={styles.optionGrid}>
          <label>
            <input
              ...
              onChange={(event) =>
                setRunCalibration(event.target.checked)
              }
            />
            確率補正
          </label>
          <label>
            <input
              ...
              onChange={(event) =>
                setRunDiagnosis(event.target.checked)
              }
            />
            診断生成
          </label>
...
export default function AiControlCenterPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const jobsBody = await readJsonResponse(
        jobsResponse,
        "ジョブ取得に失敗しました",
      );
    ...
    return (
      <main className={styles.loginPage}>
        <form className={styles.loginCard} onSubmit={saveKey}>
          <span className={styles.eyebrow}>BOAT STRIKERS AI ULTIMATE v13</span>
          <h1>AIコントロールセンター</h1>
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.statusGrid}>
        <div className={styles.statusCard}><span>実行待ち</span><strong>{summary?.queued_count || 0}</strong></div>
        <div className={styles.statusCard}><span>実行中</span><strong>{summary?.running_count || 0}</strong></div>
        <div className={styles.statusCard}><span>完了</span><strong>{summary?.completed_count || 0}</strong></div>
        <div className={styles.statusCard}><span>失敗</span><strong>{summary?.failed_count || 0}</strong></div>
      </section>

      <section className={styles.workerBanner}>
        <div><span>Windows AI Worker</span><strong>{activeJobs.some((job) => job.status === "running") ? "処理中" : "待機中"}</strong></div>
        <div><span>最終完了</span><strong>{formatDateTime(summary?.last_completed_at)}</strong></div>
        <div><span>履歴予測</span><strong>{systemStatus?.predictionCounts?.history || 0}</strong></div>
        <div><span>未来予測</span><strong>{systemStatus?.predictionCounts?.upcoming || 0}</strong></div>
        <div><span>最新診断日</span><strong>{systemStatus?.latestDiagnosisDate || "—"}</strong></div>
      </section>

      <section className={styles.section}>
        <div className={styles.sectionHeading}>
          <div><span className={styles.eyebrow}>CSV UPLOAD</span><h2>データ登録</h2></div>
          <p>最大50MB・Supabase直送</p>
        </div>
      ...
      <section className={styles.section}>
        <div className={styles.sectionHeading}>
          <div><span className={styles.eyebrow}>PROCESS QUEUE</span><h2>処理状況</h2></div>
          <p>5秒ごとに自動更新</p>
        </div>
            ...
            {jobs.map((job) => (
              <article className={styles.jobCard} key={job.id}>
                ...
                <div className={styles.progressTrack}>
                  <div className={styles.progressFill} style={{ width: `${job.progress_percent || 0}%` }} />
                </div>

                {job.worker_name && <p className={styles.workerName}>Worker: {job.worker_name}</p>}
                {job.error_message && <pre className={styles.jobError}>{job.error_message}</pre>}
                {job.status === "completed" && job.result_summary && Object.keys(job.result_summary).length > 0 && (
                  <pre className={styles.resultSummary}>{JSON.stringify(job.result_summary, null, 2)}</pre>
                )}
              </article>
            ))}
          </div>
