...
function NoteFeatureCta({ noteFeature }) {
  if (!noteFeature?.is_published || !noteFeature?.note_url) return null;
  ...
  return (
    <a
      ...
      className={styles.noteFeatureCard}
    >
      <span className={styles.noteFeatureBadge}>🔥 PICK UP</span>
      <div>
        <small>{noteFeature.character_name || "一果"}の詳細版</small>
        <strong>{noteFeature.feature_title || "このレースをnoteで詳しく解説中"}</strong>
        <p>{noteFeature.teaser_text || "前日データ・狙い目・危険ポイントを深掘りしています。"}</p>
      </div>
      <b>noteで読む →</b>
    </a>
  );
...
function PredictionPanel({ prediction, title, noteFeature = null }) {
  if (!prediction) {
    return (
      <>
        <EmptyAi
        ...
        />
        <NoteFeatureCta noteFeature={noteFeature} />
      </>
    );
  }
  ...
  const bets = Array.isArray(prediction.bet_json)
    ? prediction.bet_json
    : [];
  const marks = Array.isArray(prediction.marks) ? prediction.marks : [];
  const factors = prediction.factors && typeof prediction.factors === "object"
    ? prediction.factors
    : {};
  const factorLabels = {
    national: "全国勝率",
  ...
  const scoreDelta = prediction.score_delta === null || prediction.score_delta === undefined || prediction.score_delta === ""
    ? null
    : Number(prediction.score_delta);
  ...
  return (
    <div className={styles.predictionPanel}>
      ...
      {scoreDelta !== null && Number.isFinite(scoreDelta) && (
        <div className={`${styles.predictionChange} ${scoreDelta > 0 ? styles.predictionChangeUp : scoreDelta < 0 ? styles.predictionChangeDown : ""}`}>
          <span>前日版 {number(prediction.score_before, 0)}%</span>
          <b>→</b>
          <strong>展示補正 {scoreDelta > 0 ? "+" : ""}{scoreDelta}pt</strong>
          <b>→</b>
          <span>直前版 {number(prediction.score, 0)}%</span>
        </div>
      )}
      ...
      {marks.length > 0 && (
        <div className={styles.predictionMarks}>
          {marks.map((item) => (
            <div key={`${item.mark}-${item.boat_no}`}>
              <span>{item.mark}</span>
              <BoatBadge boatNo={item.boat_no} />
              <strong>{item.boat_no}号艇</strong>
              <small>{racerName(item.racer_name)}</small>
            </div>
          ))}
        </div>
      )}
...
function buildOfficialExhibitionAnalysis(entries = []) {
  ...
  return {
    ...
    officialRanks: {
      ...
      lap: createRanks(
        rows,
        (row) => row.official_lap ?? row.lap_time
      ),

      turn: createRanks(
        rows,
        (row) => row.official_turn ?? row.turn_time
      ),

      straight: createRanks(
        rows,
        (row) => row.official_straight ?? row.straight_time
      ),
...
function buildExhibitionAnalysis(entries = [], venueBaselines = {}) {
  ...
  const raceAverages = {
    exhibition: median(
      safeEntries.map((entry) => entry.exhibition_time)
    ),
    lap: median(
      safeEntries.map((entry) => entry.official_lap ?? entry.lap_time)
    ),
    turn: median(
      safeEntries.map((entry) => entry.official_turn ?? entry.turn_time)
    ),
    straight: median(
      safeEntries.map(
        (entry) => entry.official_straight ?? entry.straight_time
      )
    ),
    averageSt: median(
      safeEntries.map((entry) => entry.average_st)
    ),
  };

  const baselines = {
    exhibition: finiteNumber(venueBaselines?.exhibition),
    lap: finiteNumber(venueBaselines?.lap),
    turn: finiteNumber(venueBaselines?.turn),
    straight: finiteNumber(venueBaselines?.straight),
  };

  // 公式値 − 当該レース中央値 ＋ 場基準値。
  // 場基準値が未登録の項目は、公式値をそのまま補正値として表示します。
  const correctToVenue = (officialValue, raceAverage, venueBaseline) => {
    const official = finiteNumber(officialValue);
    if (official === null) return null;

    if (raceAverage === null || venueBaseline === null) {
      return official;
    }

    return official - raceAverage + venueBaseline;
  };
  ...
  const rows = safeEntries.map((entry) => {
    ...
    const correctedExhibition = correctToVenue(
      entry.exhibition_time,
      raceAverages.exhibition,
      baselines.exhibition
    );
    const correctedLap = correctToVenue(
      entry.official_lap ?? entry.lap_time,
      raceAverages.lap,
      baselines.lap
    );
    const correctedTurn = correctToVenue(
      entry.official_turn ?? entry.turn_time,
      raceAverages.turn,
      baselines.turn
    );
    const correctedStraight = correctToVenue(
      entry.official_straight ?? entry.straight_time,
      raceAverages.straight,
      baselines.straight
    );
    ...
    if (
      predictedStart !== null &&
      averageSt !== null &&
      raceAverages.averageSt !== null
    ) {
      const averageStCorrection = Math.max(
        -0.03,
        Math.min(
          0.03,
          (averageSt - raceAverages.averageSt) *
    ...
    const venueDifference = (correctedValue, baselineValue) =>
      correctedValue !== null && baselineValue !== null
    ...
    return {
      ...
      corrected_lap: correctedLap,
      corrected_turn: correctedTurn,
      corrected_straight: correctedStraight,
      predicted_start: predictedStart,

      venue_diff_exhibition: venueDifference(
        correctedExhibition,
        baselines.exhibition
      ),
      venue_diff_lap: venueDifference(correctedLap, baselines.lap),
      venue_diff_turn: venueDifference(correctedTurn, baselines.turn),
      venue_diff_straight: venueDifference(
        correctedStraight,
        baselines.straight
      ),
    };
  ...
  return {
    ...
    correctedRanks: {
      ...
      lap: createRanks(rows, (row) => row.corrected_lap),
      turn: createRanks(rows, (row) => row.corrected_turn),
      straight: createRanks(rows, (row) => row.corrected_straight),
      start: createRanks(rows, (row) => row.predicted_start),
    },
  };
}
  ...
  return (
    <div style={{ display: "grid", gap: "20px" }}>
      <ExhibitionTable
        ...
        columns={[
          ...
          },
          {
            key: "lap",
            label: "一周",
            getValue: (row) => row.official_lap ?? row.lap_time,
            getRank: (row) =>
          ...
          },
          {
            key: "turn",
            label: "まわり足",
            getValue: (row) => row.official_turn ?? row.turn_time,
            getRank: (row) =>
          ...
          },
          {
            ...
            label: "直線",
           getValue: (row) =>
              row.official_straight ?? row.straight_time,
            getRank: (row) =>
      ...
      <AnimatedStartSlit
        ...
        description="公式の展示STとF・L判定をもとに、スリット隊形を再現します。"
        note="※ F艇はF値に応じてスタートラインを越えて進み、赤いF表記で表示します。"
      />
...
function CorrectedExhibitionSuite({
  entries,
  event,
  venueBaselines: suppliedVenueBaselines = null,
}) {
  /*
  ...
  return (
    ...
    >
      ...
      <AnimatedStartSlit
        entries={rows}
        valueKey="predicted_start"
        markKey="exhibition_fl"
        title="BoatStrikers補正スタートアニメーション"
        eyebrow="BOATSTRIKERS PREDICTED START"
        description="展示ST・平均STなどをもとに補正した予測スリットです。"
        note="※ 予測STは0.01〜0.30の範囲で表示します。公式のF・L判定は引き継がず、スタートラインを越えて表示しません。"
        showStartMarks={false}
        allowLineOverrun={false}
        clampMin={0.01}
        clampMax={0.30}
      />
