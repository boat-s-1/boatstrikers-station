...
export const LIVE_STATES = ['実データ確認済み','接続済み・開催待ち','診断準備済み・開催待ち','PC-KYOTEI補完中','公式対象項目なし'];
export const VENUES = names.map((name,i)=>{
 const code=i+1, verified=[2,5,6,10,12,14,15,18,19,20,21,22,23,24].includes(code), legacy=[4,7,9,11,13,16,17].includes(code);
 const stage=verified?'接続済み':legacy?'既存取得経路':code===8?'公式休止中':code===3?'公式対象項目なし':'調査中';
 const operationalState=code===1?'診断準備済み・開催待ち':[2,22].includes(code)?'接続済み・開催待ち':verified||legacy?'実データ確認済み':code===8?'PC-KYOTEI補完中':'公式対象項目なし';
 return {code,name,stage,operationalState,
  endpoint:code===1?'/api/admin/official-exhibition-probe':verified?'/api/admin/verified-exhibition-check':null,
  straightAbsent:[12,13,18].includes(code),
  note:code===2?'開催日・R固定の出走表XMLと独自展示XMLを全国公式の6艇・登録番号・展示タイムと照合。4項目対応。展示公開後の実値確認待ち。':code===22?'固定HTTPS取得先を接続。公式6艇の登録番号・展示タイムを全国公式と照合。4項目対応。展示公開後の実値確認待ち。':[6,21].includes(code)?'公式の開催日・R・6艇の登録番号・展示タイムを全国公式と照合。4項目対応。':code===10?'開催中の公式ページで選択R・6艇の登録番号と独自3項目を照合。':code===24?'公式の進入コース・選手名・展示タイムを全国公式の艇番・登録番号と照合。4項目対応。':code===18?'HTTPS公式出走情報と6艇の登録番号・展示タイムを照合。直線なし。':[19,20].includes(code)?'公式HTTPSと全国公式直前情報で日付・R・6艇の登録番号・展示タイムを照合。4項目対応。':code===12?'公式の艇番・登録番号・展示タイムを全国公式と照合。直線なし。':code===23?'当日限定。翌日ページへの切替に注意。':code===5?'公式出走情報と6艇を照合。':code===8?'公式発表により機材トラブルでオリジナル展示データを当面休止。PC-KYOTEI補完を使用。':code===3?'公式オリジナル展示項目を確認できないため、通常展示またはPC-KYOTEI補完を使用。':code===1?'公式は半周ラップ・まわり足・直線を提供。取得構造と艇番照合を調査中。':legacy?'既存の公式取得処理あり。この画面の個別診断は未接続。':'取得先・照合方法を調査中。',
 };
});
export function diagnosticStatus(body) {
 if(body?.kiryu?.classification==='rank_only')return {label:'順位のみ取得',tone:'warn',detail:'公式順位を取得。実測タイム欄へは保存しません。'};
 if(body?.kiryu?.classification==='measurement_candidate')return {label:'実測値候補',tone:'warn',detail:'開催中に6艇・登録番号・展示タイムの照合が必要です。'};
 if(body?.kiryu?.classification==='not_published')return {label:'開催・展示待ち',tone:'warn',detail:'公式ページは非開催または展示未公開です。'};
 if(body?.kiryu?.classification==='structure_unconfirmed')return {label:'構造確認待ち',tone:'warn',detail:'期待する独自展示項目を確認できません。'};
