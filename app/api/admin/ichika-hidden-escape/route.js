...
function calcStats(rows) {
  const alerts = rows || [];
  const finished = alerts.filter((row) => Number.isFinite(Number(row.result_rank)) && Number(row.result_rank) > 0);
...
function buildDaily(rows) {
  ...
  for (const row of rows || []) {
    if (!map.has(row.race_date)) map.set(row.race_date, []);
    map.get(row.race_date).push(row);
  }
...
async function enrichRows(supabase, rows) {
  ...
  const [{ data: entries, error: entryError }, { data: events, error: eventError }] = await Promise.all([
    supabase
    ...
    supabase
      .from("bs_race_events")
      .select("race_date,course_code,race_no,result_available,trifecta,trifecta_payout,trifecta_popularity")
      .gte("race_date", minDate)
      .lte("race_date", maxDate),
  ]);
  ...
  const entryMap = new Map((entries || []).map((x) => [keyOf(x), x]));
  const eventMap = new Map((events || []).map((x) => [keyOf(x), x]));
  ...
  return rows.map((row) => {
    ...
    return {
      ...row,
      result_rank: entry?.arrival_order ?? null,
      result_available: Boolean(event?.result_available || entry?.arrival_order),
      trifecta: event?.trifecta ?? null,
      trifecta_payout: event?.trifecta_payout ?? null,
      trifecta_popularity: event?.trifecta_popularity ?? null,
    };
  });
}
...
export async function GET(request) {
  try {
    ...
    const fields = "id,race_date,course_code,course_name,race_no,closing_time,racer_class,exhibition_time,exhibition_rank,exhibition_best_time,exhibition_gap,lap_time,lap_rank,detected_at,notified,notified_at";
    ...
    const { data: allRowsRaw, error: allError } = await supabase
      .from("bs_ichika_hidden_escape_alerts")
      .select(fields)
    ...
    const history = await enrichRows(supabase, allRowsRaw || []);
    ...
    return NextResponse.json({ ok: false, error: error?.message || "取得に失敗しました" }, { status: 500 });
  }
}
...
export async function POST() {
  try {
    const supabase = getSupabase();
    const { data, error } = await supabase.rpc("evaluate_ichika_hidden_escape_alerts");
    if (error) throw error;
    return NextResponse.json({ ok: true, inserted: Number(data || 0) });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error?.message || "判定に失敗しました" }, { status: 500 });
  }
}
