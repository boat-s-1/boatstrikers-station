...
function exhibitionDiagnosisMessage(score) {
  const value = finiteNumber(score);

  if (value === null) return "公式展示情報を集計中です。";
  if (value >= 96) return "展示気配は抜群です。直前版で強く評価できる内容です。";
  if (value >= 91) return "展示内容は非常に良好。本命候補として高く評価できます。";
  if (value >= 86) return "展示内容は良好。前日版から評価を上げられる気配です。";
  if (value >= 81) return "展示気配は平均以上。最終判断の有力材料になります。";
  if (value >= 75) return "展示はまずまず。相手関係も合わせて確認したい内容です。";
  return "展示で大きな上積みは見られません。慎重な判断が必要です。";
}

function getLatestExhibitionUpdatedAt(entries = [], event = null) {
  const candidates = [
    event?.exhibition_synced_at,
    event?.api_synced_at,
    event?.synced_at,
    event?.updated_at,
...
function formatExhibitionUpdatedTime(value) {
...
function getExhibitionUpdateState(entries = [], event = null) {
  const rows = Array.isArray(entries)
    ? entries.filter((entry) => {
        const boatNo = Number(entry?.boat_no ?? entry?.teiban);
        return Number.isInteger(boatNo) && boatNo >= 1 && boatNo <= 6;
      })
    : [];

  const expectedCount = rows.length >= 6 ? 6 : rows.length || 6;

  const anyCount = rows.filter((entry) =>
    [
  ...
  const completedCount = rows.filter((entry) => {
    const exhibitionTime = finiteNumber(entry?.exhibition_time);
    const exhibitionStart = getOfficialExhibitionStart(entry);

    return exhibitionTime !== null && exhibitionStart !== null;
  }).length;

  const updatedAt = getLatestExhibitionUpdatedAt(rows, event);
...
function ExhibitionUpdateStatus({ state }) {
  const updatedTime = formatExhibitionUpdatedTime(state?.updatedAt);
    ...
    return (
      <div
        style={{
          ...
          background:
            "linear-gradient(135deg, #f1fff7 0%, #e5f9ee 100%)",
          color: "#17663f",
          boxShadow: "0 8px 22px rgba(31, 137, 82, .10)",
        }}
      >
        <div
          style={{
        ...
        >
          全艇の展示タイム・展示STを取得し、最新の展示診断へ反映しました。
          {updatedTime && ` 最終更新 ${updatedTime}`}
        </p>
      </div>
    );
  }
    ...
    return (
      <div
        style={{
          ...
          background:
            "linear-gradient(135deg, #f2faff 0%, #e6f5ff 100%)",
          color: "#205f8e",
          boxShadow: "0 8px 22px rgba(40, 126, 183, .08)",
        }}
      >
        <div
          style={{
        ...
        >
          {state.completedCount}/{state.expectedCount}艇を取得済みです。
          全艇分が揃うと、展示診断と最終買い目を表示します。
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      style={{
        ...
        background:
          "linear-gradient(135deg, #fffdf4 0%, #fff7df 100%)",
        color: "#825c18",
        boxShadow: "0 8px 22px rgba(151, 106, 20, .08)",
      }}
    >
      <div
        style={{
      ...
      >
        公式展示情報の公開後、展示タイム・展示STを取得して直前版を更新します。
        現在は展示診断を表示していません。
      </p>
    </div>
  );
...
function IchikaLivePanel({ prediction, previousPrediction, entries, event, noteFeature }) {
  ...
  const exhibitionUpdateState = getExhibitionUpdateState(entries, event);
  const isExhibitionComplete =
    exhibitionUpdateState.status === "complete";

  const analysis = buildExhibitionAnalysis(entries, venueBaselines);
  const ratings = isExhibitionComplete
    ? buildExhibitionRatings(
        analysis.rows,
        analysis.correctedRanks
      )
    : [];

  const best = ratings[0] || null;
  const previousMain =
    Number(previousPrediction?.main_boat) || null;
  const liveMain = Number(prediction?.main_boat) || null;

  const scoreDiff =
    finiteNumber(prediction?.score) !== null &&
    finiteNumber(previousPrediction?.score) !== null
      ? finiteNumber(prediction.score) -
        finiteNumber(previousPrediction.score)
      : null;
  ...
  const downgradedBoat =
    factorValue(
      prediction,
      "downgraded_boat",
      "evaluation_down_boat"
    ) ?? null;

  const exhibitionScore =
    finiteNumber(
      factorValue(
        prediction,
        "exhibition_score",
        "exhibition_index",
        "live_exhibition_score"
      )
    ) ??
    finiteNumber(best?.score) ??
    finiteNumber(prediction?.score);

  const exhibitionGrade =
    factorValue(
      prediction,
      "exhibition_grade",
      "exhibition_rank"
    ) ??
    (exhibitionScore === null
      ? "-"
      : scoreToGrade(exhibitionScore));

  const exhibitionStars = scoreToStars(exhibitionScore);
  ...
  return (
    <div className={styles.predictionPanel}>
      ...
      <ExhibitionUpdateStatus state={exhibitionUpdateState} />

      {isExhibitionComplete ? (
        <>
          <div
            style={{
              marginTop: "14px",
              ...
              background:
                "linear-gradient(135deg, #f4fbff 0%, #e7f6ff 55%, #f8fcff 100%)",
              display: "grid",
              gridTemplateColumns: "minmax(0, 1fr) auto",
              gap: "16px",
              alignItems: "center",
              boxShadow:
                "0 8px 24px rgba(31, 132, 204, .10)",
            }}
          >
            <div>
              <span
              ...
              >
                {exhibitionDiagnosisMessage(exhibitionScore)}
              </p>
                  ...
                  style={{
                    ...
                    background:
                      scoreDiff >= 0
                        ...
                        : "rgba(202, 55, 55, .10)",
                    color:
                      scoreDiff >= 0 ? "#14804a" : "#bd3636",
                    fontSize: "12px",
                    fontWeight: 900,
                  }}
                >
                  前日比 {scoreDiff > 0 ? "+" : ""}
                  {scoreDiff.toFixed(0)}点
                </div>
              )}
            </div>
          ...
          >
            <MetricCard
              label="展示評価"
              value={exhibitionGrade}
            />
            <MetricCard
              label="補正展示トップ"
              value={best ? `${best.boatNo}号艇` : "-"}
            />
            <MetricCard
              label="展示で評価アップ"
              value={
                upgradedBoat ? `↑ ${upgradedBoat}号艇` : "-"
              }
              accent="#16834d"
            />
            <MetricCard
          ...
          {(previousMain || scoreDiff !== null) && (
            ...
            >
              <div>
                前日本命：
                {previousMain ? `${previousMain}号艇` : "-"}
                {" → "}
                直前本命：
                {liveMain ? `${liveMain}号艇` : "-"}
              </div>
