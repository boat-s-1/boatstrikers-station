...
import { validKiryuRace } from '../../../../lib/kiryuVerifiedOriginalTenji.js';
...
export async function GET(request) {
  ...
  if (!validKiryuRace(race) && !validMarugameRace(race) && !validVerifiedRace(race) && !validTamagawaRace(race) && !validTokuyamaRace(race) && !validShimonosekiWakamatsuRace(race) && !validSuminoeOmuraRace(race) && !validHamanakoAshiyaRace(race) && !validFukuokaRace(race) && !validTodaRace(race) && !validCurrentDayVenue(race)) return reply({ ok:false, error:'invalid_or_unverified_venue' },400);
