...
export function parseProfile(html,fallback){
  const xs=lines(html);
  ...
  const name=compact(headerName)||compact(fallback?.racer_name);
  if(official!==digits(fallback?.racer_registration_no))throw new Error("profile_registration_mismatch");
...
async function fetchProfileOnce(candidate,signal){
  const official=digits(candidate.racer_registration_no);
  const url=`${PROFILE_BASE}?toban=${official}`;
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),10000);
  try{
    const res=await fetch(url,{cache:"no-store",signal:signal?AbortSignal.any([signal,controller.signal]):controller.signal,headers:{
...
export async function fetchProfile(candidate,signal){
  try{
    return await fetchProfileOnce(candidate,signal);
  }catch(firstError){
