...
function getStadiumName(code) {
  return STADIUM_NAMES[Number(code)] || `場${code}`;
}
      ...
      <div className={styles.modal} onClick={(event) => event.stopPropagation()}>
        ...
        <div className={styles.modalHeader}>
          <div>
            ...
            <h2>{getStadiumName(race.stadium_code)}・{race.race_no}R</h2>
            <span
        ...
        <p className={styles.disclaimer}>
          ※推奨買い目は、艇別成績・展示・モーターなどを使った参考候補です。
...
export default function ProductAiDashboard() {
  ...
  const stadiumOptions = useMemo(() => {
    const codes = [...new Set(races.map((race) => Number(race.stadium_code)))];

    return codes
      .filter((code) => Number.isFinite(code))
      .sort((a, b) =>
        getStadiumName(a).localeCompare(getStadiumName(b), "ja"),
      );
  }, [races]);
  ...
  const filteredRaces = useMemo(() => {
    ...
    if (filter === "top") {
      next = next.filter((race) => Number(race.star_count) >= 3);
    } else if (filter === "inside") {
      next = next.filter((race) =>
        ["イン鉄板", "イン有力", "女子戦注目"].includes(
          race.diagnosis_label,
        ),
      );
    } else if (filter === "hole") {
    ...
    if (stadiumFilter !== "all") {
      next = next.filter(
        (race) => Number(race.stadium_code) === Number(stadiumFilter),
      );
    }
    ...
    next.sort((a, b) => {
      if (sortMode === "inside") {
        return (
          Number(b.inside_expectation || 0) -
          Number(a.inside_expectation || 0)
        );
      }

      if (sortMode === "hole") {
        return (
          Number(b.hole_expectation || 0) -
          Number(a.hole_expectation || 0)
        );
      }

      if (sortMode === "danger") {
        return Number(b.danger_score || 0) - Number(a.danger_score || 0);
      }

      if (sortMode === "consensus") {
        return (
          Number(b.consensus_score || 0) -
          Number(a.consensus_score || 0)
        );
      }
    ...
    return next;
  }, [races, filter, stadiumFilter, sortMode]);
    ...
    <main className={styles.page}>
      ...
      {summary && (
        <div className={styles.dailyMessage}>
          ...
          <small>
            最終診断: {formatDateTime(summary.generatedAt)}
            <br />
      ...
      {topRace && (
        <section className={styles.heroCard}>
          <div className={styles.heroMain}>
            ...
            <h2>{getStadiumName(topRace.stadium_code)}・{topRace.race_no}R</h2>
            <span
              className={`${styles.labelBadge} ${
            ...
            {Number(topRace.total_score || 0) < 62 && (
              <p className={styles.absoluteWarning}>
                当日1位ですが、絶対評価は慎重です。
              </p>
            )}
          </div>
          ...
          <div className={styles.sortButtons}>
            {[
              ...
              <button
                type="button"
                key={value}
                className={sortMode === value ? styles.activeSort : ""}
            ...
            {filteredRaces.map((race, index) => (
              ...
              >
                <span className={styles.listRank}>
                  {sortMode === "total" && stadiumFilter === "all"
                    ? race.daily_rank
                    : index + 1}
                </span>
                ...
                <div className={styles.raceIdentity}>
                  ...
                  <strong>{getStadiumName(race.stadium_code)}・{race.race_no}R</strong>
