...
export function parseMikuniOfficialOriginalTenji(html) {
  ...
  const header = clean(source.match(/<thead\b[^>]*>([\s\S]*?)<\/thead>/i)?.[1] || "");
  if (!/<th\b[^>]*class=(['"])[^'"]*\bhalf-rap-time\b[^'"]*\1/i.test(source)
    || !/<th\b[^>]*class=(['"])[^'"]*\bcurve-time\b[^'"]*\1/i.test(source)
    || !/<th\b[^>]*class=(['"])[^'"]*\bextension-time\b[^'"]*\1/i.test(source)
    || !header.includes("半周ﾗｯﾌﾟ") || !header.includes("まわり足") || !header.includes("直線")) return [];
  const rows = [];
  for (const match of source.matchAll(/<tr\b[^>]*class=(['"])[^'"]*\bframe([1-6])\b[^'"]*\1[^>]*>([\s\S]*?)<\/tr>/gi)) {
    const classBoat = Number(match[2]), row = match[3], boatText = cellByClass(row, "frame-no");
    if (boatText === null) continue;
    const boatNo = Number(boatText);
    const racerNo = cellByClass(row, "racer-no");
...
export async function fetchMikuniVerifiedOriginalTenji(race, options = {}) {
  ...
  const url = `https://www.mikuniks-web.jp/races/${raceNo}`;
  ...
  try {
    const [response, reference] = await Promise.all([
      fetch(url, { cache: "no-store", redirect: "manual", signal: controller.signal, headers: { "user-agent": "BoatStrikers/1.0 (+https://www.boat-strike.online/)", referer: "https://www.mikuniks-web.jp/" } }),
    ...
    return {
      ...
      identity: { verified: true, courseCode: 10, raceDate, raceNo, evidence: "exact_official_race_url_plus_six_boat_racer_matches_national_reference" },
