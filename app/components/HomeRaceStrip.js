...
function statusInfo(course, clock, raceDate) {
  ...
  */
  ...
  const racesWithClosingTime = races
    .map((race) => ({
      ...race,
      closingMinutes: timeToMinutes(
        race.closingTime
      ),
    }))
    .filter(
      (race) =>
        race.closingMinutes !== null
    );

  if (racesWithClosingTime.length > 0) {
    const nextRace =
      racesWithClosingTime.find(
        (race) =>
          currentMinutes <
          race.closingMinutes
      );
    ...
    if (nextRace) {
      const remaining =
        nextRace.closingMinutes -
        currentMinutes;

      const displayTime =
        formatClosingTime(
          nextRace.closingTime
        );

      return {
        label: `${nextRace.raceNo}R ${displayTime}`,
        key:
          remaining >= 0 &&
          remaining <= 5
            ? "soon"
  ...
  */
  ...
  if (firstUnfinished?.raceNo) {
    return {
      label: `${firstUnfinished.raceNo}Rから`,
      key: "scheduled",
    };
  }
...
export default function HomeRaceStrip({
  courses = [],
  raceDate = "",
}) {
      ...
      <div className={styles.rail}>
        {visibleCourses.map(
          (course) => {
            ...
            const href =
              `/races/${code}?date=${raceDate}`;

            return (
              <Link
                href={href}
                className={`${styles.card} ${styles[type.key]} ${
                  status.key === "finished"
                    ? styles.finished
                    : ""
                }`}
