...
const STAGES = [
  ...
  ["dashboard", "Dashboard"], ["notification", "通知"],
];
...
export default function AiControlCenterV14() {
  ...
  if (!adminKey) return <main className={styles.loginPage}><form className={styles.loginCard} onSubmit={(e) => { e.preventDefault(); const v = keyInput.trim(); if (v) { sessionStorage.setItem(STORAGE_KEY, v); setAdminKey(v); } }}><span className={styles.eyebrow}>BOAT STRIKERS AI ULTIMATE v14</span><h1>AI Pipeline Engine</h1><p>AI管理キーを入力してください。</p><input type="password" value={keyInput} onChange={(e) => setKeyInput(e.target.value)} /><button>開く</button><Link href="/bsc2/admin">管理画面へ戻る</Link></form></main>;

  return (
    <main className={styles.page}>
      <header className={styles.header}><div><span className={styles.eyebrow}>BOAT STRIKERS AI ULTIMATE v14</span><h1>AI Pipeline Engine</h1><p>CSVから診断まで、AI運用を一元管理します。</p></div><div className={styles.headerActions}><Link href="/bsc2/admin/ai-dashboard">診断ダッシュボード</Link><Link href="/bsc2/admin">管理画面</Link><button onClick={loadData}>{loading ? "更新中…" : "更新"}</button><button onClick={() => { sessionStorage.removeItem(STORAGE_KEY); localStorage.removeItem(STORAGE_KEY); setAdminKey(""); }}>ログアウト</button></div></header>
