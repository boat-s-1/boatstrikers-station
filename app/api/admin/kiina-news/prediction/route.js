...
export async function GET(request) {
  ...
  if (rankingError) {
    return NextResponse.json({ ok: false, error: "ranking_read_failed" }, { status: 500 });
  }
  ...
  if (predictionError) {
    return NextResponse.json({ ok: false, error: "prediction_read_failed" }, { status: 500 });
  }

  const probability = Number(ranking.probability);
  const chance = Number.isFinite(probability) ? (probability * 100).toFixed(1) : "";
