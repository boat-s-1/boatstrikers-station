...
export async function fetchAmagasakiOfficialOriginalTenji(race, options = {}) {
  ...
  try {
    ...
    const selected = [...html.matchAll(/<a\b[^>]*class=(['"])[^'"]*\bselected\b[^'"]*\1[^>]*>/gi)];
    const matching = selected.filter((link) => new RegExp(`data-day=(['"])${raceDate}\\1`, "i").test(link[0])
      && new RegExp(`data-race=(['"])${raceNo}\\1`, "i").test(link[0])
      && /data-req=(['"])cyokuzen\1/i.test(link[0]));
    const identityOk = matching.length >= 1;
    if (!identityOk) {
