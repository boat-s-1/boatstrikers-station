  ...
  return (
    <div className={styles.predictionPanel}>
      <div className={styles.predictionHero} style={{ background: "linear-gradient(135deg, #fff7eb 0%, #ffe0b4 100%)" }}>
        <div>
          <span className={styles.aiEyebrow}>ICHIKA IN-ESCAPE PRE-DAY AI</span>
          <h3>一果AI 前日版</h3>
          <small>1号艇のイン逃げ条件を6項目で診断</small>
        </div>
      ...
      >
        一果独自のイン逃げ診断です。★が多いほど、1号艇の逃げを後押しする条件が揃っています。
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "10px", marginTop: "14px" }}>
        <IchikaStarMetric
          label="AI期待値"
          score={metrics.aiScore}
          detail={metrics.aiScore === null ? null : `${Math.round(metrics.aiScore)}%`}
        />
        <IchikaStarMetric
          label="イン逃げ期待度"
          score={metrics.escapeScore}
          detail={metrics.escapeScore === null ? null : `${Math.round(metrics.escapeScore)}%`}
        />
        <IchikaStarMetric
          label="1号艇モーター評価"
