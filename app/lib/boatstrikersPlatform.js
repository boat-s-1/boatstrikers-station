...
export async function getCourseRaces(raceDate, courseCode) {
  ...
  const supabase = getSupabaseServerClient();
  const { data, error } = await supabase
    .from("v_bs_race_page").select("*")
    .eq("race_date", normalizedDate).eq("course_code", normalizedCourseCode)
    .order("race_no", { ascending: true }).order("boat_no", { ascending: true });
  if (error) throw new Error(`レース一覧の取得に失敗しました: ${error.message}`);
    ...
    if (!grouped.has(raceNo)) {
      ...
      grouped.set(raceNo, {
        ...
        race_status: event.result_available
          ? "result"
          : hasValue(row.exhibition_time) || hasValue(row.exhibition_st)
            ? "exhibition"
            : "program",
        entries: [],
      });
    }
    grouped.get(raceNo).entries.push(mapEntry(row));
  }
...
export async function getRaceDetail(raceDate, courseCode, raceNo) {
  ...
  const { data, error } = await supabase
    .from("v_bs_race_page").select("*")
    .eq("race_date", normalizedDate).eq("course_code", normalizedCourseCode)
    .eq("race_no", normalizedRaceNo).order("boat_no", { ascending: true });
  if (error) throw new Error(`レース詳細の取得に失敗しました: ${error.message}`);
  ...
  const result = event.result_available || resultEntries.length
    ? {
