...
import { addPremiumRole, discordConfig, ensureDefaultNotificationRoles, getAdminClient, isDiscordEligible, joinGuild } from "../../../../lib/discordPremium";
...
export async function GET(request){
  ...
  try{
    ...
    const tokenResponse=await fetch("https://discord.com/api/v10/oauth2/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:tokenBody,cache:"no-store"});
    ...
    const meResponse=await fetch("https://discord.com/api/v10/users/@me",{headers:{Authorization:`Bearer ${tokenData.access_token}`},cache:"no-store"});
    ...
    const {data:profile,error:profileError}=await admin.from("bs_member_profiles").select("plan,membership_status").eq("user_id",stateRow.user_id).maybeSingle();
    ...
    await ensureDefaultNotificationRoles(discordUser.id);

    const {error:linkError}=await admin.from("bs_member_discord_links").upsert({
      user_id:stateRow.user_id,discord_user_id:discordUser.id,discord_username:discordUser.username||null,
      discord_global_name:discordUser.global_name||null,discord_avatar:discordUser.avatar||null,
      linked_at:now,last_role_synced_at:now,last_role_state:"premium",updated_at:now,
    },{onConflict:"user_id"});
