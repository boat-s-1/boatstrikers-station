...
export default function SeminarMagazineShelf({ magazine }){
  const basePath=magazine.basePath||`/library/${magazine.slug}`;
  const archiveTitle=magazine.collectionLabel||"バックナンバー";
  const guideTitle=magazine.collectionLabel?"本の読み方":"雑誌の読み方";
  return <main className={`${styles.page} ${styles[magazine.accent]}`}>
    <header className={styles.header}><Link href="/" className={styles.logo}>BOAT<br/><span>STRIKERS</span></Link><Link href={magazine.homeHref||"/library"} className={styles.libraryLink}>{magazine.homeLabel||"図書館へ戻る"}</Link></header>
    ...
    <section className={styles.shelfSection}><div className={styles.sectionTitle}><div><span>BOOK ARCHIVE</span><h2>{archiveTitle}</h2></div><p>{magazine.issues.length}冊</p></div><div className={styles.grid}>{magazine.issues.map((issue,index)=><Link href={`${basePath}/${issue.id}`} className={styles.issueCard} key={issue.id}><div className={styles.coverWrap}>{index===0&&<span className={styles.newBadge}>NEW</span>}{issue.cover?<img src={issue.cover} alt={`${magazine.name} ${issue.number}`} className={styles.cover}/>:null}<span className={styles.readBadge}>読む</span></div><div className={styles.issueInfo}><span>{issue.number}</span><h3>{issue.title}</h3><time>{formatDate(issue.date)}</time></div></Link>)}</div></section>
    <section className={styles.guide}><strong>{guideTitle}</strong><p>表紙をタップするとビューアが開きます。スマホは左右スワイプ、PCは左右ボタン・矢印キーでページを送り、その下で文章による詳しい解説を読めます。</p></section>
  </main>;
}
