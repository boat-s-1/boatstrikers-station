...
import RacesSeoResolvedDate from './RacesSeoResolvedDate';
import styles from './racesSeo.module.css';

export default function RacesSeoIntro({ raceDate, courseCount = null, courseNames = [], femaleRaceCount = 0 }) {
  const visibleCourses = courseNames.slice(0, 8);
  const hasCourseCount = courseCount !== null && courseCount !== undefined && courseCount !== '';
  const courseCountText = hasCourseCount && Number.isFinite(Number(courseCount)) ? `開催場は${Number(courseCount)}場です。` : '';
    ...
    <section className={styles.wrap} aria-labelledby="races-seo-title">
      ...
      {visibleCourses.length > 0 && (
        <p className={styles.courses}>
          <strong>主な開催場：</strong>{visibleCourses.join('・')}{courseNames.length > visibleCourses.length ? ' ほか' : ''}
        </p>
      )}
      ...
      <div className={styles.cards}>
        <article>
          <span>01</span>
          <h2>開催場と出走表を確認</h2>
          <p>開催場を選ぶと、各レースの出走表や締切時刻へ進めます。表示中の日付の開催場から確認してください。</p>
        </article>
        <article>
          <span>02</span>
