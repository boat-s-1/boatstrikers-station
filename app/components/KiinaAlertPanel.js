...
export default async function KiinaAlertPanel(){
  ...
  return <section style={{margin:"18px 14px",borderRadius:24,overflow:"hidden",background:"#fff",boxShadow:"0 8px 24px rgba(141,105,20,.10)",border:"2px solid #e8bc48"}}>
    <div style={{position:"relative",padding:"18px 18px 16px",background:"linear-gradient(135deg,#fff0a8 0%,#ffe28b 45%,#fff 100%)",borderBottom:"1px solid #f1ddb0"}}>
      ...
      <h2 style={{margin:"4px 0 2px",fontSize:25,color:"#4c3a12",lineHeight:1.15}}>4→5展開理論</h2>
      <div style={{fontSize:12,fontWeight:800,color:"#765d22"}}>4号艇の展示・直線が上位になった狙い目を検知</div>
    ...
    {todayRows.length?<div style={{padding:"14px 14px 8px",display:"flex",gap:12,overflowX:"auto",scrollSnapType:"x mandatory",WebkitOverflowScrolling:"touch"}}>
      {todayRows.map(a=><article key={a.id} style={{flex:"0 0 88%",scrollSnapAlign:"start",border:"1px solid #f0dfac",borderRadius:17,padding:14,background:"#fffdf7",boxSizing:"border-box"}}>
        <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center"}}><strong style={{fontSize:20,color:"#17345c"}}>{a.course_name||`${a.course_code}場`} {a.race_no}R</strong><span style={{padding:"6px 9px",borderRadius:999,background:"#fff0c7",color:"#a96b00",fontSize:12,fontWeight:900}}>条件成立</span></div>
    ...
    </div>:<div style={{padding:"20px 18px 22px",textAlign:"center"}}><div style={{fontSize:30}}>🚨</div><strong style={{display:"block",marginTop:6,color:"#17345c"}}>現在、条件成立レースはありません</strong><p style={{margin:"7px 0 0",fontSize:13,color:"#718096",lineHeight:1.6}}>展示情報が出た後、4→5展開理論の条件が成立すると表示されます。</p></div>}
