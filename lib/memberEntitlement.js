...
function emptyEntitlement(){
  return {authenticated:false,active:false,betaOpen:false,plan:"free",plus:false,premium:false,user:null,profile:null};
}
...
export async function requireMemberEntitlementFromRequest(request,{level="active"}={}){
  ...
  if(!entitlement.authenticated){
    return {ok:false,status:401,error:"会員ログインが必要です。",entitlement};
  }
  if(!entitlement.active){
    return {ok:false,status:403,error:"有効なBoatStrikers会員のみ利用できます。",entitlement};
  }
  if(level==="plus"&&!entitlement.plus){
    return {ok:false,status:403,error:"この機能はPREMIUM対象会員限定です。",entitlement};
  }
  if(level==="premium"&&!entitlement.premium){
    return {ok:false,status:403,error:"この機能はPREMIUM対象会員限定です。",entitlement};
  }
