...
export async function generateDailyResultDigest({ date } = {}) {
  ...
  const [{ data: allEvents, error: eventError }, { data: entries, error: entryError }] = await Promise.all([
    client.from("bs_race_events")
      .select("course_code,course_name,race_no,race_name,race_kind_code,trifecta,trifecta_payout,winning_technique_code,result_available,race_cancel_code")
  ...
  if (!allEvents?.length) throw new Error(`${raceDate} のレースデータがありません。`);

  const expectedEvents = allEvents.filter((x) => !String(x.race_cancel_code || "").trim());
  const events = expectedEvents.filter((x) => x.result_available === true);
  if (!events.length) throw new Error(`${raceDate} の確定結果がありません。`);
  if (events.length !== expectedEvents.length) {
    throw new Error(`${raceDate} は結果未確定です（${events.length}/${expectedEvents.length}R）。記事公開を見送ります。`);
  }
  ...
  const eventKeys = new Set(events.map((x) => `${x.course_code}-${x.race_no}`));
  const winnerEntries = (entries || []).filter((x) => x.arrival_order === 1 && eventKeys.has(`${x.course_code}-${x.race_no}`));
  ...
  const venueManshuCounts = {};
  for (const item of manshu) venueManshuCounts[item.venue] = (venueManshuCounts[item.venue] || 0) + 1;
  ...
  const dataLab = {
    ...
    venue_manshu_counts: venueManshuCounts,
