...
const SELECT_COLUMNS = "race_date,course_code,race_no,prediction_timing,mode_key,mode_name,is_hit,hit_ticket,tickets,ticket_count,investment,payout,profit,recovery_rate,result_combination,settled_at";
...
async function fetchAllRows({supabase,range,course,mode,timing}){
...
export default async function AiBetPublicResultsPage({searchParams}){
  ...
  let safeRows=[];
  try{safeRows=await fetchAllRows({supabase:getClient(),range,course,mode,timing});}
  ...
  const selectedTimingName=TIMING_META[timing]?.label||timing;
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.summarySection}>
      <div className={styles.sectionHeading}><div><p>PERFORMANCE</p><h2>{periodLabel(period)}・{selectedTimingName}の成績</h2></div><span>{summary.races}レース集計</span></div>
