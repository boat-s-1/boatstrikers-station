...
function buildLineText(alert){const remaining=minutesUntil(alert.race_date,alert.closing_time);const remainingText=remaining===null?"":`\n締切まで約${Math.max(0,Math.ceil(remaining))}分`;const closing=formatClosingTime(alert.closing_time);const closingText=closing?`\n締切 ${closing}`:"";const raceUrl=`https://www.boat-strike.online/races/${alert.course_code}/${alert.race_no}`;return ["🚨【4号艇ダブル上位】",`${alert.course_name||""} ${alert.race_no}R`,`④ 展示 ${alert.exhibition_time??"-"}【${alert.exhibition_rank??"-"}位】`,`④ 直線 ${alert.straight_time??"-"}【${alert.straight_rank??"-"}位】`,`${closingText}${remainingText}`.trim(),"BoatStrikersでレース詳細を見る",raceUrl].filter(Boolean).join("\n");}
