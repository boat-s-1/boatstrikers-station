      ...
      <section className={arrivalStyles.section} aria-labelledby="library-arrivals-title">
        <div className={arrivalStyles.titleBox}>
          <img
            src="/IMG_6319.jpeg"
            alt="新刊入荷のお知らせ"
            className={arrivalStyles.banner}
          />
        </div>
        ...
        {news.length > 0 ? (
          <>
            <a
              ...
              className={arrivalStyles.featured}
            >
              <div className={arrivalStyles.featuredCover}>
                <img src={news[0].image} alt="" />
                <span>NEW</span>
              </div>

              <div className={arrivalStyles.featuredBody}>
                <div className={arrivalStyles.featuredMeta}>
                  <time dateTime={new Date(news[0].date).toISOString()}>
                    {new Date(news[0].date).toLocaleDateString("ja-JP", {
                      month: "numeric",
                      day: "numeric",
                    })}
                  </time>
                ...
                <p className={arrivalStyles.featuredCategory}>{news[0].category}</p>
                <h2 id="library-arrivals-title">{news[0].displayTitle}</h2>
                <span className={arrivalStyles.readButton}>この新刊を読む <i>›</i></span>
              </div>
            </a>

            {news.length > 1 && (
              <div className={arrivalStyles.recent}>
                <div className={arrivalStyles.recentHeading}>
                  <h3>最近の入荷</h3>
                  <span>NEW ARRIVALS</span>
                </div>

                <div className={arrivalStyles.list}>
                  {news.slice(1).map((n, index) => (
                    <a
                      href={n.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className={arrivalStyles.item}
                      key={n.link}
                    >
                      <time dateTime={new Date(n.date).toISOString()}>
                      ...
                      <span className={arrivalStyles.thumb}>
                        <img src={n.image} alt="" />
                      </span>

                      <span className={arrivalStyles.itemText}>
                        <strong>{n.category}</strong>
                        <small>{n.displayTitle}</small>
                      </span>
            ...
            <a
              className={arrivalStyles.allLink}
          ...
          <div className={arrivalStyles.empty}>
            <span>📚</span>
            <strong>新刊情報を準備中です</strong>
            <p>新しい記事が入荷すると、こちらに表示されます。</p>
          </div>
        )}
      </section>
