-- BoatStrikers AI v2 storage compaction.
-- ai_v2_training_rows is regenerable. Remove duplicated JSON snapshots and keep scalar features only.
-- IMPORTANT: run only when the database is writable.

truncate table public.ai_v2_training_rows;

drop function if exists public.ai_v2_refresh_training_rows_base(date,date,text);

alter table public.ai_v2_training_rows
...
begin
  ...
  with result_base as (
    select
      ...
      trim(coalesce(rr.winning_method, rr.winning_technique, '')) as winning_method_text
  ...
  parsed_results as (
    select
      r.*,
      split_part(r.trifecta_text,'-',1)::smallint as parsed_first_boat,
      split_part(r.trifecta_text,'-',2)::smallint as parsed_second_boat,
      split_part(r.trifecta_text,'-',3)::smallint as parsed_third_boat
    from result_base r
    where r.trifecta_text ~ '^[1-6]-[1-6]-[1-6]$'
  ),
  raw_entries as (
    select
      e.race_date,e.course_code,e.race_no,e.boat_no,
      ev.race_day_no,ev.wind_speed,ev.wave_height,
      e.racer_registration_no,e.racer_class,e.class,e.gender,e.gender_code,e.sex_code,
      e.national_win_rate,
      ...
      e.flying_count,e.late_count,e.course1_average_st,
      ...
      pr.parsed_first_boat,pr.parsed_second_boat,pr.parsed_third_boat,pr.winning_method_text,
      coalesce(
        rgm.racer_gender,
        case
    ...
    where e.race_date between p_start_date and p_end_date
      and pr.parsed_first_boat<>pr.parsed_second_boat
      and pr.parsed_first_boat<>pr.parsed_third_boat
      and pr.parsed_second_boat<>pr.parsed_third_boat
  ),
  race_quality as (
  ...
  inserted as (
    insert into public.ai_v2_training_rows(
      ...
      national_win_rate,national_2_rate,local_win_rate,local_2_rate,average_st,motor_2_rate,boat_2_rate,
      flying_count,late_count,course1_average_st,course1_top2_rate,course1_race_count,
      ...
      finish_position,is_first,is_second,is_third,ichika_target,hatsune_target,kiina_target
    )
    select
      f.race_date,f.course_code,f.race_no,f.boat_no,p_data_timing,
      'ai-v2-feature-v2','historical_reconstructed',null,
      f.race_day_no,f.is_female_race,coalesce(f.class,f.racer_class),
      f.national_win_rate,f.national_2_rate,f.local_win_rate,f.local_2_rate,f.average_st,f.motor_2_rate,f.boat_2_rate,
      f.flying_count,f.late_count,f.course1_average_st,f.course1_top2_rate,f.course1_race_count,
      ...
      case when f.boat_no=f.parsed_first_boat then 1 when f.boat_no=f.parsed_second_boat then 2 when f.boat_no=f.parsed_third_boat then 3 else null end,
      f.boat_no=f.parsed_first_boat,
      f.boat_no=f.parsed_second_boat,
      f.boat_no=f.parsed_third_boat,
      case when f.boat_no=1 then case when f.parsed_first_boat=1 and regexp_replace(f.winning_method_text,'[[:space:]　]+','','g')='逃げ' then 1 else 0 end end,
      case when f.boat_no=1 and f.is_female_race then case when f.parsed_first_boat=1 then 1 else 0 end end,
      case when f.boat_no=5 then case when f.parsed_first_boat=5 then 1 else 0 end end
    from features f
...
comment on function public.ai_v2_refresh_training_rows(date,date,text) is
  'Rebuilds compact leak-safe AI v2 training rows. Scalar features only; no duplicated JSON snapshot.';
