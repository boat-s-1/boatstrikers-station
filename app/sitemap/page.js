...
export const metadata = {
  ...
  openGraph: {
    title: "サイトマップ｜BoatStrikers",
    description: "BoatStrikersの主要ページを、目的別に分かりやすくご案内します。",
    url: "/sitemap",
    type: "website",
  },
};
...
const sitemapGroups = [
  ...
  },
  {
    ...
    links: [
      ...
      { title: "初音の女子戦攻略マガジン", description: "女子戦を楽しむための情報をまとめています。", href: "/library/hatsune-seminar" },
      { title: "キイナの穴党塾", description: "高配当や穴狙いの考え方を学べます。", href: "/library/kiina-seminar" },
      { title: "教えて！一果センセー", description: "ボートレースの基本を動画で分かりやすく解説します。", href: "/ichika-sensei" },
    ],
  },
  {
    title: "見る・聴く",
    ...
    color: "purple",
    links: [
      { title: "ふなけん研究部", description: "学校を舞台にしたボートレース漫画・ショート動画です。", href: "/comic" },
      { title: "ボート・ナイト・ニッポン", description: "一果・初音・キイナが届けるラジオ番組です。", href: "/radio" },
      { title: "番組表", description: "動画、ラジオ、noteなどの配信予定を確認できます。", href: "/schedule" },
    ],
  },
  {
    title: "サイト案内",
    ...
    color: "green",
    links: [
      { title: "BoatStrikersについて", description: "サイトのコンセプトや3人の担当メンバーをご紹介します。", href: "/about" },
      { title: "サイトマップ", description: "BoatStrikersの主要ページを一覧でご案内します。", href: "/sitemap" },
      { title: "お問い合わせ", description: "ご質問やご連絡はこちらからお送りください。", href: "/contact" },
      { title: "プライバシーポリシー", description: "個人情報やアクセス情報の取り扱いについてご案内します。", href: "/privacy" },
      { title: "利用規約", description: "BoatStrikersをご利用いただく際の規約です。", href: "/terms" },
      { title: "免責事項", description: "予想情報や掲載データに関する注意事項です。", href: "/disclaimer" },
    ],
  },
];
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.hero}>
        <div className={styles.heroDecoration} aria-hidden="true"><span>★</span><span>⚓</span><span>★</span></div>
        <span className={styles.eyebrow}>SITE MAP</span>
        <h1>サイトマップ</h1>
        <p>BoatStrikersのコンテンツを、<br className={styles.mobileBreak} />目的別に分かりやすくご案内します。</p>
      </section>

      <nav className={styles.quickNav} aria-label="サイトマップ内メニュー">
        {sitemapGroups.map((group, index) => (
          <a key={group.title} href={`#group-${index}`}><span aria-hidden="true">{group.icon}</span>{group.title}</a>
        ))}
      </nav>

      <div className={styles.groupList}>
        {sitemapGroups.map((group, groupIndex) => (
          <section id={`group-${groupIndex}`} key={group.title} className={`${styles.groupSection} ${styles[`groupSection_${group.color}`]}`}>
            <div className={styles.groupHeading}>
              <div className={styles.groupIcon} aria-hidden="true">{group.icon}</div>
              <div><span>{group.english}</span><h2>{group.title}</h2></div>
            </div>
            <div className={styles.linkGrid}>
              {group.links.map((link) => (
                <Link key={link.title} href={link.href} className={styles.linkCard}>
                  <div className={styles.linkCardText}><h3>{link.title}</h3><p>{link.description}</p></div>
      ...
      <footer className="siteFooter">
        <div className="siteFooterInner">
          <nav className="footerSitemap" aria-label="フッターサイトマップ">
            ...
            <div className="footerLinkGroup">
              <h2>サイト案内</h2>
              <a href="/about">BoatStrikersについて</a><a href="/sitemap">サイトマップ</a><a href="/contact">お問い合わせ</a><a href="/privacy">プライバシーポリシー</a><a href="/terms">利用規約</a><a href="/disclaimer">免責事項</a>
            </div>
          </nav>
          <div className="footerNotice">
            <p>当サイトに掲載している予想や情報は、的中および利益を保証するものではありません。</p>
            <p>舟券の購入は20歳になってから。無理のない範囲でボートレースをお楽しみください。</p>
          </div>
        </div>
      </footer>
    </main>
