...
export async function GET(request) {
  ...
  if (!validMarugameRace(race) && !validVerifiedRace(race) && !validTamagawaRace(race) && !validTokuyamaRace(race) && !validCurrentDayVenue(race)) return reply({ ok:false, error:'invalid_or_unverified_venue' },400);
