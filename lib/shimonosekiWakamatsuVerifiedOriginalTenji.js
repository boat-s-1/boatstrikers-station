...
export async function fetchShimonosekiWakamatsuVerifiedOriginalTenji(race, options = {}) {
  ...
  // Shimonoseki's official endpoint is consistently slower from Vercel than Wakamatsu's.
  // Keep the longer allowance venue-specific so it cannot delay the other collector.
  const defaultTimeoutMs = Number(race.courseCode) === 19 ? 16000 : 10000;
  const maximumTimeoutMs = Number(race.courseCode) === 19 ? 18000 : 12000;
  const timer = setTimeout(
    () => controller.abort(),
    Math.min(maximumTimeoutMs, Math.max(1000, Number(options.timeoutMs) || defaultTimeoutMs)),
  );
  try {
