...
function parseHeiwajima(html) {
  ...
  for (let boatNo = 1; boatNo <= 6; boatNo += 1) {
    ...
    rows.push({ ...originalRow, exhibitionTime: exhibitionTimes.get(boatNo) ?? null });
  }
  return rows;
}
...
function parseMiyajima(responseText) {
  const parts = String(responseText || "").split("####");
  const html = parts[7] || "";
  const text = htmlToText(html);
  if (!text.includes("一周") || !text.includes("まわり足") || !text.includes("直線")) return [];

  const rows = [];
  for (const cells of parseTableRows(html)) {
    const boatNo = Number(cells[0]);
    if (!(boatNo >= 1 && boatNo <= 6) || cells.length < 8) continue;
    const exhibitionTime = numericOrNull(cells[4]);
    const lapTime = numericOrNull(cells[5]);
    const turnTime = numericOrNull(cells[6]);
    const straightTime = numericOrNull(cells[7]);
    if ([exhibitionTime, lapTime, turnTime, straightTime].every((v) => v === null)) continue;
    rows.push({ boatNo, exhibitionTime, lapTime, turnTime, straightTime });
  }
  return [...new Map(rows.map((row) => [row.boatNo, row])).values()].sort((a, b) => a.boatNo - b.boatNo);
...
const ADAPTERS = {
  ...
  17: {
    key: "miyajima_official",
    label: "BOAT RACE宮島公式",
    requests({ raceDate, raceNo }) {
      return [{
        ...
        body: new URLSearchParams({ race: String(Number(raceNo)), date: ymd(raceDate) }).toString(),
      }];
    },
    parse: parseMiyajima,
  },
...
export async function fetchOfficialOriginalTenji(race, options = {}) {
  ...
  if (!adapter) {
    return { ok: false, supported: false, courseCode, source: null, rows: [], error: "official_adapter_not_verified" };
  }
  ...
  try {
    const requests = adapter.requests
      ? adapter.requests(race)
      : (adapter.urls?.(race) || []).map((url) => ({ url, method: "GET" }));
