...
export default function MinamoComicPromptBuilder({ payload, newsItems = [] }) {
  ...
  }, [payload, mode, comicType, topic, protagonist, punchline, audience, dailyIdea, news, selectedCandidate]);
    ...
    <section id="minamo-comic" style={{ scrollMarginTop: 80, marginTop: 28, padding: 20, borderRadius: 18, border: "1px solid rgba(255,255,255,.12)", background: "rgba(7,18,38,.72)" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ margin: 0, opacity: 0.78, lineHeight: 1.7 }}>実データ回に加えて、ボートレースあるある・学園日常・キャラ掛け合いも同じ画面から作れます。</p>
        </div>
      ...
      <div style={{ marginTop: 14, padding: 12, borderRadius: 12, background: "rgba(255,255,255,.05)", fontSize: 13, lineHeight: 1.7, opacity: 0.82 }}>
        <strong>使い分け：</strong> ニュース・実結果は「ファクト重視」、普段の学園4コマは「ネタ重視」、実データを少し混ぜる日常回は「バランス型」がおすすめです。
      </div>
